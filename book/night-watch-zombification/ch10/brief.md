# Brief — Chapter 10, "The Ninth Hind"

Book: night-watch-zombification. POV: Vesk. Culture: Accord. Scene type: working. Proposed Front: project-zombification. Day 12, the twelfth division. Assembled 2026-09-24 from outline.json, bible.md, state.json, the ch9 final and the read-only book tools. Nothing here is written to the table.

## The beat

The outline row, verbatim:

```json
{
 "number": 10,
 "title": "The Ninth Hind",
 "beat": "At night in the coldhouse the ninth hind, dead twenty-three days, stands up off the table and leans across the slate toward the capped standpipe housing in the corner, where Malphas has been sitting on the gauge bench; he rises and walks to the door before she reaches it, and Vesk takes it for courtesy. She lies down against the housing and stays down. Deming, present to hear what his money bought, is sick in the yard; Foss writes the distance and the time; Malphas says the sink is the quiet. The matrix walks.",
 "scene_type": "working",
 "pov": "Vesk",
 "cast": [
  "Vesk",
  "Malphas",
  "Ilmar Foss",
  "Nuvalik",
  "Xu Deming"
 ],
 "place": "Greyshaft Nine, the coldhouse",
 "front_id": null,
 "proposed_front": "project-zombification",
 "plants": [],
 "pays": [
  "H21-the-ninth-hind"
 ],
 "time_elapsed": "Day 12, the twelfth division",
 "target_words": 3500
}
```

**Three binding continuity readings (decided here; the row's words yield to the canon text where they differ).**

1. **She has been opened.** Ch6 ends on Day 5 with Nuvalik taking "the big knife off the hook to start the ninth hind's hide off her, because a hind that had done nothing in five days was a hind you learned from by opening." Nothing since (ch8 does not mention her) says she was finished or taken off the table. *Working reading:* Nuvalik started the hide off the near shoulder and flank that night and stopped there; the ninth hind has lain on the slate since, opened along that side, under sacking. When she stands, the drawn tissue over the scapula is bare to the lamp. The chapter says in one line why Nuvalik stopped, or it does not say; it may not have her flayed whole or butchered, and it may not have her rehung.
2. **Where Malphas sits.** The row puts him "on the gauge bench" in the corner by the standpipe housing. In the canon the gauge bench is Foss's, on the east wall in the ice press's warm air (ch2: "The press threw its warm air along the east wall and the gauge bench stood in the middle of it"; ch8: "Somebody had left the gauge bench squared, his glass in its box"). The dead standpipe housings are on the **west** wall (ch2, ch6), and ch8 seats Malphas "on the iron bench under the dead standpipes". *Working reading:* Malphas sits on the iron bench under the dead standpipes, at the west wall's corner housing. Call it that. Do not call it the gauge bench, and do not move the gauge bench. Vesk's ch2 misreading (the press's warm air explains the Lord's hands) is his and stands; this seat is out of the press's air, and Vesk may see that the hands still have their colour there (see the Protocol, ignorance quota). He makes nothing of it.
3. **The day count (audit C9).** The row says "dead twenty-three days". Ch2 gives "Eleven days on the rail" on Day 1, and ch2 contradicts itself on the twelfth day. *Working reading:* no age for her is printed. Foss writes the date and the division, not her days. If a count must reach the page it is a private count in Vesk's head, and it does not fight ch2.

### Hooks this chapter must plant

None (outline `plants: []`).

### Hooks this chapter pays

- **H21-the-ninth-hind** (planted ch2, advanced ch6, due ch10). Keywords: `ninth hind`, `table`, `stood`, `standpipe`. Text: "'The ninth hind is the freshest and she is the one I want.' Paid when she stands up off the table and leans to the standpipe." Events so far: ch2, "The ninth hind is the freshest and she is the one I want."; ch2, the hide at her shoulder drew up once, untouched, and Vesk told nobody; ch6, "The hide over the scapula lay the way hide lies." The pay: she stands up off the slate table, leans across the slate toward the capped standpipe housing, and lies down against it and stays down. Use the hook's words on the page: table, stood, standpipe. Pay it in full here.

### Hooks the beat touches without paying (advance only if the page earns it; never pay)

- **H07-warm-hands** (planted ch2, due ch60, advanced ch6, ch8). Keywords: `hands`, `warm`, `cold`, `gallery`. Every crew POV has seen it once (Vesk ch2, Nuvalik ch6, Foss ch8), and each explained it away or stopped looking. In ch10 it is Vesk's again, from a new seat (continuity reading 2). It may be seen. It is never explained, never connected to the hind's lean, and never paid.
- **H10-vesks-grafts** (planted ch2, due ch46, advanced ch6). Keywords: `grafts`, `nails`, `stones`, `grave`. Vesk's hands close two thirds of the way; he carries in both flat palms; the yard gloves; the two ridged yellow nails grown out again since the fourth day; the village's beck stones on graves. A dead thing getting up in his room is the first time the village's stones have a use he can see. He may think of the stones. Nobody lays one. Do not pay it: it pays at his death, ch46.
- **H11-the-sky-finishes** (planted ch2, due ch38; audit: dormant since ch2). Keywords: `sky`, `ground`, `ravens`, `walk out`. Nuvalik's saying can come back in Vesk's head or her mouth ("Where I was raised, the sky finishes what the ground starts, and when the sky refuses, people walk out to see why."). Do not pay it (Tally's walk, ch38).
- **H09-nuvaliks-three-names** (planted ch2, due ch55). Keywords: `name`, `three`, `set down`, `leaves`. Touch only if the page earns it. She does not leave.
- **H16-thirty-one-days** (planted ch4, due ch22). Keywords: `thirty-one`, `days`, `counted`, `useful`. Deming's count is private (ch4) and Vesk cannot know it. Nothing on the page counts it aloud. Deming may be seen with his hat, "thumb round the brim".
- **H18-the-project-name-is-the-file** (planted ch4, due ch57). Keywords: `Project Zombification`, `word`, `file`, `chose`. The night goes in the book "under the name I gave it, by four hands" (ch2). The word may be written. Nothing is paid.
- **H28-foss-released** (planted ch6, due ch70). Foss writes the distance and the time. It is his hand in the book again. Do not touch the promise unless the page earns a line.

### Hook agenda check

H21 is due and paid here. No other hook is due by ch13. No hook is overdue: H08 (due 6) was paid in ch6 and H01 (due 7) in ch7. The full agenda is under Continuity.

### Front tick (proposed Front, from bible.md)

#### project-zombification (Malphas's cell) — 6 segments
- **Want:** a dose that walks with no anchor, no binding and no name, until the substrate is spent.
1. **The dose walks.** The contractile matrix proven on the table. Ch2, 6, 10 (the ninth hind stands).
2. **Release.** The dead loose in the field and the edges gone. Ch8, 13, 17 (the Bureau's bleed re-classification read and accepted).
3. **Human substrate.** The salt bar in the parish, the pit walks. Ch24, 27, 29.
4. **The Accord builds the sinks.** Enforcement's night cuts harnessed; the cost is Vesk. Ch34, 38, 43, 45, 46.
5. **The cell comes apart.** Deming let fall, a season waited, Nuvalik gone. Ch50, 55.
6. **Ended, and kept.** The project closed, Greyshaft Nine burned, Foss released; the method survives in Salter's stillroom. Ch66, 68, 70, 73, 75, 78, 80.

**This chapter's tick:** segment 1, "The dose walks", third and last push: **ch10 fills it** (ch2 the pour, ch6 the contraction reproduced under house rules, ch10 the stand). Mention is not advancement (bible). Here is how the world changes. A dose with no anchor, no binding and no name gets a dead animal off a table and across a room, and it goes to the emptiest thing there, the capped standpipe. Every POV in the room sees the consequence: the hind against the housing, the distance and the time in Foss's hand, Deming in the yard. The want is proven on the table, not yet in the field. Segment 2 (Release) begins ch13; ch11 moves the dose into salt. Nothing in ch10 releases anything into the field, decides the salt bar, or names the Wicket Lane contract (that is ch11's, from Deming's side). `front_id` stays null. Nothing is written to `table/fronts.yaml`, which needs `add_front` on Isaac's word.

## Protocol

`desktop/NATALIE.md` governs the drafting, with these chapter adaptations:

- **No turn-taking.** Table Rule 1's first half (answer what the PC did, then stop at his next decision) is off, because this is a chapter and not a turn. Its ending rule stays: end on an NPC line, a physical action, or a thing the POV can now see. Never end on a question aimed at the reader or at Isaac.
- **No PC in this book.** The standing book rule from bible.md takes the PC's place: **neither Wystan nor Malphas ever learns of the other.** Malphas knows his hunter only as "the Night Register", a form and an institution. Nothing noticed across that line is ever paid as recognition, and the narration never winks at a reader who knows both sides. The voice note's anti-exemplar is exactly what is forbidden. **Specific to ch10:** the lean goes to the lowest-saturation thing in the room. Bible, Flags: "The book lets Vesk see Malphas step away from the ninth hind's lean once (ch10) and never explains it." So: the hind's first line of lean may be toward the corner where Malphas sits; he gets up and walks to the door before she reaches it; she goes on to the housing. The narration never says why, never says Voidic, Shell, saturation of a man, or anything about his body, and never sets it beside any other man. Wystan, Cutler Row and anything on the Accord side of the line stay off the page. If the Register comes up at all (the carter's news of ch8 is known to Malphas and Foss), it is "the Register", a form with no face, and nothing in this chapter connects a lean to it.
- **POV and interiority.** POV is **Vesk**, close, in his craft (voice note: "Vesk's formulation"): the make-up, the dilution, the flask carried in two flat palms, the drip counted to twenty-two. R39-7 binds: only Vesk thinks in italics. Nuvalik, Foss, Deming and Malphas get no private italic thought. **Malphas is seen, never entered** (voice note), and he stays opaque: not large, fills nothing when he arrives or moves, forearms on his knees, coat folded beside him and not under him. No card figure, no Stage, no Remnant reaches the page (bible). **One object handled** (voice note: one object per chapter): suggestion, the flask, or the yard gloves on hands that do not close.
- **Length:** 3,500 words (outline target), set-piece band. Table Rule 2 sets a set piece at 2,500 minimum with full scene standards. Scene type is working: the mechanism is on the page, so the rule loadout carries magic-mechanism, combat and codex. There is no fight. Blanks 10, 11 and 13 of the scene brief are "none: no contest" unless the writer finds one. Blank 12 (two of the four explaining voices) applies: the mechanism is carried by Nuvalik's anatomy and Foss's instrument, and Malphas's one sentence ("the sink is the quiet") is a statement, not an explanation.
- **Mechanism.** Bible: run **wotr-phenomenon** before drafting ch10. Established on the page: "a tissue that shortens when one end of her is emptier than the other. Put the empty end somewhere and she goes there. Not because she chooses it. Because that is what shortening does." (Nuvalik, ch2); "An anchor is a binding. A binding takes a name." and "a thing with no name in it cannot be told where to go. It can only be given somewhere emptier than where it is." (Malphas, ch2); Vesk, ch2: "this building is the emptiest place in four streets" and "Eleven feet is all she needs." / Malphas: "She will need to get up, which she has not done." The stand pays that exchange. Foss's distance is an instrument's number. If the writer prints it, it may be eleven feet (Vesk's ch2 figure, now measured). Any other figure is originated and must be declared in the notes. Malphas touches neither the dose nor the substrate (house rule three, ch6: "nothing comes from a flask to me, or from the substrate to me, without a second pair of hands between"). He does not touch her. His ch6 admission that he put his hand on her flank the first night "stops now". This is a second reason Vesk can give himself for the Lord's getting up (courtesy, the house rule), and both are his.
- **One deliberate lie by an NPC and one misreading by the POV character** (Table Rule 8; R6-4, R5-C1). Declare both verbatim in the notes. Neither is corrected on the page.
  - **Misreading (Vesk): the row's own.** Malphas rises and walks to the door before she reaches him, and Vesk takes it for courtesy, a man giving the room to the work. It must be well reasoned from what Vesk has: the Lord's manners, house rule three, the Lord's standing at the door through the eighth hind's pours in ch6 ("Malphas was at the door with his coat over his arm"). It stays wrong and is never corrected, in this chapter or any other (bible: never explained). It sits beside Vesk's standing ch2 misreading (the press explains the hands), and the page may lean on that one.
  - **Lie.** The liar is an NPC present in the chapter. Candidate, the writer's choice: **Deming**, back from the yard, gives a cause for being sick that is not the hind ("the river road," "something at the ford"). It is harmless, in character (ch4's "Cause: the thaw" is his house style of cover), and changes no later chapter. Nuvalik already carries a ch2 lie. Foss's instrument readings must stay true: H20 and the hearing depend on the book. Do not give the lie to Malphas. A lie from him would have to be read as a lie, and he stays opaque.
- **Deming.** He is in the cast "present to hear what his money bought". In canon the Verdantia was not bought: it was drawn down as wastage on the Swale supply contract (ch4, source: "I cannot buy two myriads. I diverted it."). The phrase is Vesk's or the outline's; nobody in the room says the dose was bought. Audit: "Deming trips at ch10 unless ch10, where he is in the cast, gives him a fact." Give him one on the page: the hat, the thumb round the brim, careful diction, what he sees before he goes out. He has been to the coldhouse before (ch4: Malphas "sent for him to the coldhouse after the ninth division"). Ch11 (Day 13) is his POV and moves the dose into salt bars. Nothing in ch10 decides that, or speaks of the parish-relief contract.
- **The Standing Inventory** (two recurrences minimum, bible list). Greyshaft Nine has no hum: "The room had no hum in it. The main under Greyshaft Nine had been capped four years" (ch2). **The hum, and the moment it stops** is here as its absence, the book's clock read by its lack. Deming comes in off a main and may feel the lack in his soles, as Foss did in ch8. On hand: **the standpipe, iron, waist-high, capped** (the housing she lies against); **the gauge-housing and its quarterly inspection** (Foss's housing, "the seal broken on a quarterly inspection that had stopped coming nine years ago"); **the meter, its seal and the bill**, by absence ("There was no meter by the door"; "capped and unbilled"); **the roll and the entry in ink** (the book over the press, the night written up under the project's name). Recommended pair: the capped standpipe and the missing hum.
- **Numbers.** Invent none for any card. All five cards here are stubs "Pending Isaac" except Malphas's, which is the lich form and does not reach the page (bible). Numbers reach the page only in a mouth, an instrument, a document or a private count (R14-4, R12-5). Attested and usable: four degrees on the gauge; the drip at twenty-two a minute; nine on the rail; four years capped; the count of forty; four pour places; eleven feet (Vesk's, ch2). No age for the hind (continuity reading 3). Time in divisions only (audit C16).
- **Standing canon to respect.** The three house rules are cut on the door board, with the season and the wall under a rule across (ch6, ch8). Everything goes through a second pair of hands. No live dose is touched bare. The yard gloves are the only pair. Ferriby is stopped for a season and the licks are going to the Swale walks (ch8); that news may be in the room. The eighth hind was poured sealed on Day 5 to reproduce the contraction by Day 6. No chapter records the result, and this one does not supply it: she is on the rail with the rest, or unmentioned. No chapter may say the rail's nine came from Ferriby (audit C14). Vesk's ch6 lie (it "can be tempered sealed") stands uncorrected.
- **Ending.** The row's last sentence is the turn: "The matrix walks." End on a physical action, an NPC line, or a thing Vesk can now see: the hind down against the housing, the entry in Foss's hand, Malphas's line. Never end on a question.

The Standing Inventory block, from `book_tools.py session_start "Wystan Ashmore" working Accord` (the "# STANDING INVENTORY" section only):


#### STANDING INVENTORY

#### THE STANDING INVENTORY: THE GUILD ACCORD AND THE CONCORD

Note: the sources give no Concord greeting, no everyday swearing and no idiom for the items marked (d); each is derived in one step from an attested fact (the live main, the witness clause, the cased weight, the silver token, the freeman by redemption, the tongue nobody dreams in) and chosen as the plainest option. Everything else is attested in the wiki mirror.

Chartered, metered and entered. A people who did not conquer anybody and are the only layer that works in every court at once, and charge for it. **Food:** the assize loaf (weight fixed against the price of grain; a short loaf is the commonest charge on any market roll), white array-ground bread and dark hand-ground bread, and which one is on the table says what the household could afford, pulse porridge, ale at the fixed price assayed by sworn tasters, the salt bar (money that can be eaten), grounding tea for anyone who has sat too long near an Echo. Meat is a feast-day matter. **Greeting:** title before name, without exception, at any formal occasion (Prime Warden, Upper Bailiff, Master); the silver token shown at a door before the name is given (d); between neighbours the diagnostic is "Still on the main?" (d), and the answer tells you the district's pressure. **Oaths:** "Ita spondeo," thus I pledge, the only first person the chancery permits; "before witnesses" as the everyday emphatic (d); "under the seal"; Measurewrights swear the Hexagonal Oath; Enforcement and Strategic personnel carry three Oaths cut into their bones at initiation, "We see corruption, not race, creed, or kin," "Our lives are weights upon the scale," "We burn only when law demands." **Insults:** "slag," "a small button" (less to him than his reputation), "a blank" (no craft), "corded" (unreliable), "cocked" (cut off, out of the trade), "a dead leg" (on the books, does no work), "a mouth" (a speaker), "potboy" (a Drafter), "assayed" (found out), "cased" (looks solid, rings dull; d), "bought his freedom" (a freeman by redemption; d). **Time:** the Guild hour, fourteen equal hours of sixty counts pushed down the wire from Stannvaard; the turn of fourteen days ("within one turn" for any registration); Auren's day for the dead, Veyra's for markets, Irath's for musters, a court sitting on Uurgath's has chosen it; the quarterly inspection of the gauge-housing; seven years for an indenture; the Lantern's twelve years for a term of service; a man who has seen the Weight return twice is old; the Nameless day, on which nothing binds; maintenance between three and five in the morning; Year 715 of the Imperial Age, and nobody dreams in it. **The dead:** nine days above ground in the house, the sitting in shifts, and the sitting is the rite; the layer-out, who knows within the hour whether the Echo has gone; the parish roll; consecrated parish ground on Auren's day; "In registro relatum," the death entered; the excluded at the crossroads, without the roll; a death-house may not be cut for arrears; those who die in Accord service are sung each solstice, and a soldier who dies defending a Wellspring is not reclaimed. **Objects:** the meter on the wall by the door and the Board's seal on it; the standpipe, iron, waist-high, capped; the gauge-housing, brass, slate-backed, locked; the tundish; the stopcock and the cut plate; the inspector's seal press, gauge and book; the market inspector's small hammer; the silver rank token; the proof-mark, PROB EST; the strike-board; the bronze reach set into the wall at hand height; the split tally; the board in the hall with its green, blue, gold and obsidian wax; the nine-word wire; the letter of credit; the harness that cost two seasons' surplus. **Exchange:** draughts and the bill, monthly, to a chartered Board; the silver mark of twenty-four copper, and copper is the price of being alive; a letter of credit against a factor house; shares, not wages, on a Commission, with the bond posted and the losses on the subscribers; bottomry for a party without a patron; amercement set to the offender's means; fees for attestation; the entry in ink; mine scrip where the copper was melted; the tally between people who cannot read. **Proverbs, fixed wording:** "Balance before Dominion." "One weight, one measure, one reach, through all the land." "No one is measured in his own tongue." "What has been said stands said." "A clause improved is a clause void." "Where coin fades, Merit endures." "Merit spent is memory lost." "The Accord cannot hang you." "A melted coin does not testify." "A solid weight rings when struck." "Nobody dreams in Latin." (d) **Body:** the ear (the hum under a district, the dull note of a cased weight), the hands (ink and wax), sleep (children raised on a main sleep badly off it), the face at a door, which a wire-boy reads before he can read the wire.

**Recurrence, two minimum per Accord scene:** the hum, and the moment it stops; the meter, its seal and the bill; the small hammer and the dull note of a cased weight; the nine-word wire; the roll and the entry in ink.

---

## Rule loadout

`load_rules prose-law magic-mechanism combat codex --brief` (tags from `loadout working`), whole output:

-- 284 rules for tags ['codex', 'combat', 'magic-mechanism', 'prose-law']

R42-14-VIA_FATI  [Magic System Rulings 2026-09-23 follow-up]  live
  Viaforma gains Via Fati for the Fate Path.

R42-2-AETHER_CLASS_FROM_GLORY  [Magic System Rulings 2026-09-23 C-020]  live
  Aether Class emerges at Glory; Stages I–V are unclassed.

R42-7-FOUR_PATHS  [Magic System Rulings 2026-09-23 C-025]  live
  There are four Paths, Fate included.

R41-1-DISTANCE_IS_TWO_AXES  [Distance Two Axes Ruling C-015]  live
  R35-2's register (close, medium, distant/formal) says whose idiom the narration runs in; Pack Twenty's band (1 to 5) says how deep inside the POV it sits. Both assignments stand and neither trades off the other: Cozbi runs distant/formal at band 5.

R39-7-NO_NPC_THOUGHT_UNDER_POV_LOCK  [Stat System and Scene Rulings 2026-09-13 C-008]  live
  The narration-distance rule wins over Pack One's Scene Standards carve-out: a named NPC gets no private italic thought in a scene with a POV lock. The carve-out survives only where there is no lock — omniscient narration and mass combat.

R36-1-YASOSHIMA_SINK_YUNO_SECRET  [Yasoshima Essence Sink / What House Yuno Declined to Disclose What the Research Division Was Not Told]  live
  Yasoshima's Essence stability is not natural. For two hundred thousand years House Yuno's Kagura branch has used the island as a sink, gathering and metabolising the world's ambient Essence drift one convergence rite at a time through the deepest reach of Tōbō no Michi; the ore-less islands are the right vessel because nothing in them distorts what is poured in. The working is paid for: the Kagura officiants who hold each convergence spend a sliver of their own selfhood into the pattern (Ritual Self-Dilution, the cost Ayame's own card names for her single-city version at Kharven). The house refuses the Research Division because an outside apparatus measuring the sink changes what it is holding (Kiyome's bar on profane or unstable use), because a partial account is itself a badly formed act, and because the mechanism is the only leverage that has kept the island unconquered. Consequence hook if forced open: the load backs up and discharges violently at Yasoshima, a mainland Measurewright reading spikes for the first time in recorded history, the island becomes the most contested ground in the four quarters, and the house's silence recoils on whoever breaks it like a sealed vow. Reader's knowledge only; no character outside the Kagura officiant line knows it.

R35-1-NARRATION_DISTANCE_BANDS  [Narration Distances (Psychic Distance by POV) The three registers]  live
  Every POV character carries one of three narration distances — close (narration fuses with the character's own idiom, italicized direct thought available), medium (POV locked, no head-hopping, but emotion arrives by external behaviour or simile, no italicized thought) or distant/formal (an omniscient epigrammatic voice describes the character from outside, occasional aphoristic italics, the one register licensed for more narratorial explanation) — and the field is called "narration register", never "Band", because Band is the unrelated FOW Coherence stat. This supplies the whose-idiom input that R4-13-FID_CARVEOUT depends on.

R35-2-NARRATION_DISTANCE_ASSIGNMENTS  [Narration Distances (Psychic Distance by POV) Close register / Medium register / Distant-formal register / Unassigned]  live
  Close register: Darius, Aurelian, Verinus, Charles, Sodoku Moto, Niran Yukari, Wren, Kwon Mu-jin. Medium register: Rengai. Distant/formal register: Cozbi Mahuo. Each assignment carries a violation caution (never a narratorial verdict Darius does not voice himself; never validate Aurelian's certainty from outside his idiom; no neutral Church register for Verinus; no practitioner's jargon for Charles; no interpretive-summary sentences past Sodoku's procedural read; Niran's feeling only through his clinical vocabulary; Wren laconic and tactical only; Kwon Mu-jin's failing read shown from inside; no retrofitted first-person italics or verdicts for Rengai; Cozbi's explanation stays inside his self-mythologizing frame). Hild Ice and Dabney are unassigned until scenes in their own idiom exist for Isaac to rule on.

R31-1-ABYS_DEEP_ATTESTED_AT_OBLATION_FLUXIA  [Abys Deep Codex Amendment Ruling Standing Ruling]  live
  The Master Codex row for [Abys] Deep now lists Oblation among its attested Wellsprings and Fluxia among its attested Families, so a Codex assignment line drafting [Abys] on an Oblation-primary, Fluxia construct is legal as written. Its Primary Wellspring, Family and Physics Domain (Abyntheus, Fulguria, Electromagnetism) are unchanged, and [Th] Foundation is not amended and stays cross-family.

R29-1-SACRAMENT_BOND_ANCHOR_BY_DECLARATION  [Sacrament Anchor Ruling Standing Ruling]  live
  A Sacrament or Transposition anchor is a soul that holds the celebrant's name at weight, and it is fixed by the celebrant's declaration inside the rite, regardless of whether the anchor consents; the declaration, not consent, is the mechanism, and the bond is held under Fixatio, the anchoring law. It does not lapse when the celebrant leaves the world, the celebrant cannot amend it afterward, and it is not a displaceable affinity bond, so it has no release valve. Transposition.md says nothing about how an anchor bond ends; this rule is silent on ending.

R27-1-CYMORATH_AIR_OF_ASCENT_FROST_ON_VOHRIN  [Cymorath Portfolio Ruling Standing Ruling]  live
  Cymorath carries exactly the portfolio Fracture of Worlds gives it: the Air of Ascent, Family Fulguria, freedom and motion, strengthening Dexterity Celerity and Gnosis Cartography/Perception. It is not a frost Wellspring. Every frost, cold or stasis working the wiki had been drafting as Cymorath is keyed instead to Vohrin, the Abyssal Depths Titan, Family Caloria, Physics Domain Thermodynamics, on the FOW III reading that cold is heat run backward. There is no dedicated frost Wellspring. Codex lines, Wellspring harmonisation rows and Material Ledger resonance columns that name a cold law write Vohrin · Caloria · Thermodynamics; any surviving Cymorath usage is Fulguria.

R26-1-CONCORDANT_CRYSTAL_MECHANISM  [Shared Soul Crystal Mechanism 1]  live
  Establishes the Concordant Crystal as the general, reusable mechanism for a Soul Crystal shared by multiple donors: the Essence Cores stay discrete and take turns at the frame's single, non-multiplying Aether Shell; the Attraction Layer alone runs one Wellspring line per donor; Coherence and eta are pooled off the Shell while Tempering Coherence stays per-donor; the frame runs Overgrown by default; and a donor's death or removal either settles the frame toward Refined or, if the survivors will not release it, curdles into Obsession Force -- a failure mode, like single-Shell collapse, that no single-donor Crystal can suffer.

R25-1-OBSESSION_SATISFIES_ATTRACTION_GATE  [Obsession Force / Attraction Path Gate Ruling Standing Ruling]  live
  A practitioner whose Attraction Layer runs under Obsession Force — the corrupted inversion of Attraction Force — still satisfies an Attraction Path gate for sub-stat purposes. Obsession does not cap Attraction-gated sub-stats hard; this is a setting-wide mechanical rule covering every corrupted practitioner, not a fix scoped to one character.

R24-2-ELADRIN_NEVER_BARE_LINE  [Racial Voice and Dialect Guide Amendment II]  live
  Never write an Eladrin line of dialogue without a distance or stillness beat attached; a bare Eladrin line is an incomplete sentence in their language.

R24-2-ELADRIN_THREE_CHANNELS  [Racial Voice and Dialect Guide Amendment II]  live
  Prose must render at least two of three simultaneous channels in any exchange of consequence — sparse speech that rarely carries the content, distance (approach is agreement, a half-step back is objection, turning without moving the feet is refusal), and stillness (complete stillness is often the loudest thing in the room).

R23-11-NAME_AVOIDANCE_WAITING  [Inner World Naming Amendment XI]  live
  The name of the recently dead is unsayable until given on; the Waiting is the silence — the body waits in the death-house because the ground is frozen, and the name waits with it, leaving a hole in the household's vocabulary that everyone steers around.

R22-4-SYSTEM_TERMS_TABLE  [Moto Reversion Ledger IV]  live
  Ajiin→Hataraki (the Working), Nüdel→Shingan (the Seeing), Zasag→Kamigan (the Ruling Sight), Iltgel→Meigan (the Revealing Sight), Süldiin→Reigan (the Spirit Sight), Tengeriin→Tengan (the Sky Sight), Khar Ild→Kurosetsu (Black Blade), Ünen→Asami (Truth), Takhil→Gisei (Offering).

R22-5-SEVEN_WORKS_TABLE  [Moto Reversion Ledger V]  live
  Söröl Ajiin→Hametsu no Gō (Ruin), Ariun Ajiin→Junketsu no Gō (Purity), Süld Ajiin→Seirei no Gō (Spirit), Tsagiin Ajiin→Jikan no Shigoto (Time), Mergen Ajiin→Chishiki no Shigoto (Wisdom), Tegsh Ajiin→Shigoto no Baransu (Balance), Bükhel Ajiin→Zentai-sei no Hataraki (Totality).

R22-7-FUSI_VA_KEPT  [Moto Reversion Ledger VII]  live
  Fusi Vā, the Agamalu binding rite, replaced Saishiki (a Japonic word on a rite that is Agamalu in origin and Vāimoana in provenance, wrong for both registers on its own terms); the Büri amendment isn't what made that change correct, so it stays kept.

R20-4-PRONUNCIATION_ADAPTATION  [Naming Guide Amendment Part Four]  live
  When a name crosses cultures, the speaker's own phonology imposes itself (a Concord human flattens Dawi consonant gradation, a Dawi stress-accents a Yukari pitch-accent name); these adaptations should appear in dialogue as characterisation, not be treated as typos.

R20-5-FORMAL_ADDRESS  [Naming Guide Amendment Part Five]  live
  A practitioner's formal address follows the convention of whoever is doing the addressing (rank plus family name for the Accord, ordination name plus title for Sancta Lux, relationship-dependent honorifics at a Ketsuen court); two people in the same room may correctly address the same person by different names.

R20-5-ON_THE_PAGE  [Naming Guide Amendment Part Five]  live
  Introduce a character by whatever name the POV character would use (formal on first meeting, personal for intimates, Third Name or physical description for strangers); the narrator does not switch names without a reason, since a name-switch is a statement about the character's relationship to the reader.

R20C-17-SITES_RECHARGE_UNHELPFULLY  [Pack Twenty R20C-17]  live
  Sites recharge, and it does not help. Decades to centuries, and the rate falls with every working — eighty years the first time, two hundred the second.

R20C-20-WELLS_GENERATE_NO_ENTITIES  [Pack Twenty R20C-20]  live
  Wells generate no entities. The site is the entire threat.

R20C-22-AMMUNITION_TIERS_RATIFIED  [Pack Twenty R20C-22]  live
  The three ammunition tiers ratified, subject to R20C-26.

R20C-23-ORIGIN_STAYS_MYTHIC  [Pack Twenty R20C-23]  live
  Origin stays mythic. Mechanism is fully explicable; where it came from is not.

R20C-25-PERCUSSION_TABLE_DAWI_AUTHORED  [Pack Twenty R20C-25]  live
  The Percussion master-strike table is Dawi-authored in-world.

R20C-29-CATEGORY_NAMING_DIAGNOSTIC  [Pack Twenty R20C-29]  live
  Category naming in diagnostic voice: allowed.

R20C-31-SUBSTAT_NAMES_FACULTY_ONLY  [Pack Twenty R20C-31]  live
  Sub-Stat names never appear outside a faculty reading.

R20C-33-RESONANT_PAIRS_DIAGNOSTIC  [Pack Twenty R20C-33]  live
  Resonant Pair unlocks may be named in diagnostic voice.

R20C-35-CHANT_IN_OWN_LANGUAGE  [Pack Twenty R20C-35]  live
  The Latin chant rule folds into the repeal. A practitioner chants in their own language.

R20C-41-CHANTCRAFT_FIFTH_CRAFT  [Pack Twenty R20C-41]  live
  Chantcraft is the fifth craft, Ars Cantus its medium-home. Not folded.

R20C-42-LAW_V_STAGE_GATE_REFRACTION  [Pack Twenty R20C-42]  live
  Law V is a Stage gate at Refraction (VII). Below it a working needs voice, hand, ink or blood.

R20C-54-COMBAT_ASSIGNMENTS_CONFIRMED  [Pack Twenty R20C-54]  live
  The three pitched combat assignments confirmed. Black Agent: Blade, unhurried, Absetzen and Durchwechseln over committed cuts, does not engage plate.

R19-2-FIXED_TEXT  [Pack Nineteen §2]  live
  Isaac's submitted dialogue is set as written and never rewritten.

R19-2-NEVER_ALTERED  [Pack Nineteen §2]  live
  Register, rhythm, repetition, fragments, profanity, prosodic marks, line order, and word choice in Isaac's dialogue may never be changed, including for the sake of reading better.

R19-3-PROSODY_VERBATIM  [Pack Nineteen §3]  live
  Capitals, stretched vowels, ellipses and repeated punctuation are notation for volume, duration, breath and intensity, and must be reproduced exactly rather than converted into narrative description.

R19-4-REALISTIC_SPEECH  [Pack Nineteen §4]  live
  Originated dialogue must include false starts and self-repair, interruption, repetition, non-answers, characters talking past each other, trailing off, physical business mid-line, and characters saying the wrong thing, as craft law rather than optional flavour.

R19-5-COMPOSURE_BAN  [Pack Nineteen §5]  live
  Bars antithesis and parallelism, three-part lists, long subordinate clauses in shouted speech, and perfect syntax from established inarticulate/young/exhausted/foreign characters, and bars every character in a scene from constructing sentences at the same length.

R18-2-CODEX_FIRST_MANDATE  [Pack Eighteen §2]  live
  Before naming, pitching, converting or writing a working, the design order is fixed: read the Spell Index for precedent, then the Master Glyph Index, take Wellspring/Family/Physics Domain/Temperance Min/Category/Glyph Class assignment from the Lists sheet (never from memory), route alchemy to the Alchemical Index, and only then design and write.

R18-2-NO_NEW_CATEGORY_WITHOUT_CHECK  [Pack Eighteen §2]  live
  A new Wellspring, Family, Physics Domain, Archon, Titan, Glyph Class, Tier or alchemical Type may not be proposed until the existing enumeration has been read and shown not to already cover it; a proposal that does not name what was checked is not a proposal.

R18-3-CARD_IS_SPELL_INDEX  [Pack Eighteen §3]  live
  Pack Seventeen's technique card fields (Operation, Manifestation, Cost, Limit, Counter, Codex line) map exactly onto Spell Index columns (Mechanism/Trigger, Effect/Numerical-Physical-Effect/Target Response, Cost/Backlash, Temperance Min plus Mechanism's constraint, Counterplay, and Category/Primary Glyphs/Wellsprings/Family/Physics Domain respectively), and the Codex column governs.

R18-3-CODEX_OVERRIDES_LEXICON  [Pack Eighteen §3]  live
  Pack Sixteen's route-1 rough-prose conversion now means routing to a Codex term, and where the Pack Sixteen §4 conversion lexicon disagrees with the Codex sheet, the sheet wins.

R18-3-ENTRY_AS_CANDIDATE_ROW  [Pack Eighteen §3]  live
  A newly designed working's entry is written in Spell Index form and stands as a candidate row for the sheet; anything originated in play belongs in the Codex or does not exist.

R18-4-GLYPH_NAMING_CONTEXTS  [Pack Eighteen §4]  live
  Glyphs are named only in a document, an instrument, a practitioner's diagnostic voice, or a private count; narration may state what a glyph did without announcing the bracketed form.

R18-4-GLYPH_QUOTED_EXACTLY  [Pack Eighteen §4]  live
  A glyph appearing in a chant, an inscription or a Wordform Chain must be quoted exactly as the Master Glyph Index gives it.

R18-4-ROLE_VS_CLASS_AXES  [Pack Eighteen §4]  live
  Glyph Role (Authorization, Boundary, Direction, Sealing) describes function within a working; Glyph Class (Root, Derivative by Extension, Compound, Synonym, Counter) describes provenance; neither may stand in for the other.

R18-5-ALCHEMY_DOCTRINE_BINDING  [Pack Eighteen §5]  live
  Reagents are the nouns, the Wellspring is the verb, and Glyphica is the grammar; any working touching Calcination, Dissolution, Coagulatio, Distillation, Sublimare or Transmutatio is bound by Alchemetrica's entries for them.

R18-5-ALCHEMY_SOURCE_ORDER  [Pack Eighteen §5]  live
  Alchemy design consults, in order, the Alchemical Index (what exists), Alchemetrica (doctrine), and The Real Alchemy (real-world substrate check on invented process/apparatus/reagent behaviour), loading The Provenance Doctrine, The Standing Index and The Bench of Attribution alongside them; none of the three is optional.

R18-6-ACCESS_METHOD  [Pack Eighteen §6]  live
  The Master Codex is accessed read-only via openpyxl from the actual spreadsheet file; the sheet is the authority, not Natalie's recollection of it.

R18-6-SESSION_START_4A  [Pack Eighteen §6]  live
  Inserted between the current steps four and five of the Session Start Protocol, a new step 4a requires querying the Spell Index, Glyph Index, Alchemical Index and Lists sheet for everything the scene will work, before the FOW line and before the Standing Inventory.

R18-7-CHECK37  [Pack Eighteen §7]  live
  Every Wellspring, Family, Physics Domain, Temperance stage, Archon, Titan, Glyph Class, alchemical Tier and Type named in a file is validated against the Lists sheet; anything absent fails unless declared as a proposed new entry naming the checked enumeration.

R18-7-CHECK38  [Pack Eighteen §7]  live
  Every bracketed glyph token in a file is validated against the Master Glyph Index; unknown tokens fail.

R18-7-CHECK39  [Pack Eighteen §7]  live
  Manual check: for any new working, name the Spell Index rows checked and state whether the working is new, a derivation of a named row, or a duplicate.

R17-2-FOUR_ELEMENTS  [Pack Seventeen §2]  live
  Every technique entry's Effect line (or immediately under it) must name the quantity acted on, the governing law, the operation performed on the quantity, and the causal chain from trigger to outcome; missing any one makes the entry unfinished and not canon.

R17-2-THE_TEST  [Pack Seventeen §2]  live
  A technique must be statable as "it does X to Y, which under Z produces W"; if that sentence cannot be written, the technique is not designed regardless of the prose around it.

R17-3-CODEX_LINE_ORDER  [Pack Seventeen §3]  live
  The Codex line (glyphs / Wellspring / Family / Physics Domain / Category / Stage) is assigned after the phenomenon is designed, never before.

R17-3-COST_DERIVED  [Pack Seventeen §3]  live
  The Cost line must be derived from the Operation line; a cost that could be swapped for a different cost without changing the mechanism is decoration.

R17-3-COUNTER_MANDATORY  [Pack Seventeen §3]  live
  A Counter is mandatory on Signature techniques and must fall out of the Operation line; a counter that is just "hit him harder" means the technique has no mechanism.

R17-3-LIMIT_MECHANICAL  [Pack Seventeen §3]  live
  The Limit line must state why the mechanism forbids the thing it forbids; a limit with no mechanical source is a rule the author imposed rather than a derived limit.

R17-3-WHAT_NOBODY_KNOWS_SCOPE  [Pack Seventeen §3]  live
  The one-line-per-art "What nobody knows" field must be a question about why the law holds, never about what the working does; mechanism belongs on the page, origin does not.

R17-4-OUTPUTS_NOT_INPUTS  [Pack Seventeen §4]  live
  Cost, Limit and Counter must be read off the mechanism once it exists rather than chosen for drama or convenience.

R17-4-READ_OFF_MECHANISM  [Pack Seventeen §4]  live
  Design order stays what refuses/survived/costs-belief, then phenomenon, then mechanism, then Codex, unchanged from the sixth edition; Pack Seventeen adds that once the mechanism exists, cost, limit and counter are derived from it rather than chosen.

R17-5-TRAIT_SCOPE  [Pack Seventeen §5]  live
  A Trait must specify the Lattice property it alters, and therefore which injuries behave differently and which identically; a Trait that only restates its effect ("harder to kill") is not a Trait.

R17-6-MECHANISM_VS_ORIGIN  [Pack Seventeen §6]  live
  Entries must state mechanism but never need explain cosmic origin — why a Wellspring answers, where the Sixty came from, how many workings exist, or why the laws are the laws; that permanent gap is where "What nobody knows" lives.

R17-8-CHECK36  [Pack Seventeen §8]  live
  Flags phrases that assert an outcome instead of stating a process (e.g. "is assayed", "is converted", "responds to"); each is replaced with the operation it stands in for, or deleted.

R16-1-CONSEQUENCE  [Pack Sixteen §1]  live
  The technical and metaphysical register belongs in the finished narration itself, in Isaac's vocabulary, converted rather than removed or exported to notes.

R16-2-DELETION_REQUIRES_NOTE  [Pack Sixteen §2]  live
  If a rough-draft term does not survive into the finished prose in one of the four routed forms, the author notes must say which term was dropped and why, in one line, or the pass has failed.

R16-2-FOUR_ROUTES  [Pack Sixteen §2]  live
  Every content-bearing noun and verb in Isaac's rough prose is routed to a WOTR system term, a real scientific/technical term, the compound of both, or kept verbatim as a defined WOTR term; nothing is silently dropped.

R16-3-COMPOUND_SENTENCE  [Pack Sixteen §3]  live
  The signature WOTR technical sentence names the real phenomenon, gives the system term as its name/boundary/address, and states the physical consequence, in one sentence or short group; neither element is a gloss on the other.

R16-5-NOT_IN_NOTES  [Pack Sixteen §5]  live
  If a mechanism term appears in the author notes but not in the prose, the pass failed and the note is evidence of that failure.

R16-5-REGISTER_IN_NARRATION  [Pack Sixteen §5]  live
  The Technical Register is available to third-person narration for anything a person chooses to do; it is not reserved for italic thought.

R16-5-VOICES_VS_VOCABULARY  [Pack Sixteen §5]  live
  The four explaining voices remain the delivery system for causal explanation only; narration itself may carry technical terms without needing a voice to speak them.

R16-6-COINAGE_HANDLING  [Pack Sixteen §6]  live
  A term in Isaac's rough with no canon equivalent is a coinage: it is converted into a defined WOTR term, kept in the prose, given a one-line definition in the author notes, and entered on the Open Rulings Docket for ratification, never silently replaced with a plainer word.

R16-6-SPELLING_VS_VOCABULARY  [Pack Sixteen §6]  live
  Spelling and grammar in Isaac's rough are corrected without comment; vocabulary is never silently corrected.

R16-7-NOT_CONVERTED  [Pack Sixteen §7]  live
  Conversion applies to mechanism, phenomenon, technical read, injury and the physics of an exchange, but never to the body's plain physicality, grief and the elegiac beat, dialogue (outside a practitioner's diagnostic voice or a document), or the ignorance quota.

R16-7-OVERCONVERSION_FAULT  [Pack Sixteen §7]  live
  A scene that converts everything has the same fault as a scene that converts nothing: it has stopped choosing.

R15-1-AI_TELL_CHECKS_SURVIVE  [Pack Fifteen §1]  live
  The AI-tell checks (em dashes, similes, not-X-but-Y, countdown negation, gloss, ladder) are about prose failing, not about period, and are unaffected by the register repeal.

R15-1-DICTION_PALETTE  [Pack Fifteen §1]  live
  The one-or-two-elevated-words-per-scene cap is gone; the word bank remains available.

R15-1-ELEVATED_VOCAB_OPTIONAL  [Pack Fifteen §1]  live
  The Elevated Vocabulary Standard stops being a mandate; its words stay available but nobody counts them.

R15-1-LICENCE  [Pack Fifteen §1]  live
  Modern words, concepts and frames are legal in every channel of the prose, including narration and in-world documents.

R15-1-MYSTIC_REGISTER_NEVER_PHYSICS_STRUCK  [Pack Fifteen §1]  live
  Pack Twelve §6's clause forbidding a document from explaining mechanism is struck; a document may be as scientific as it wants, and the Mystic Register survives only as an option for documents that want to withhold.

R15-1-READS_MODERN_STRUCK  [Pack Fifteen §1]  live
  Natalie no longer raises, fixes, or notes "reads modern" as a critique on principle.

R15-1-REGISTER_BY_CULTURE  [Pack Fifteen §1]  live
  Per-culture narration registers become optional flavour rather than law.

R15-1-TECHNICAL_REGISTER_SURVIVES  [Pack Fifteen §1]  live
  Packs Twelve, Thirteen and Fourteen's Technical Register for combat and workings is not a register rule in this pack's sense and is unaffected; this pack makes explanation in real terms easier, not harder.

R15-1-VOICE_DIFFERENTIATION  [Pack Fifteen §1]  live
  Characters must remain non-interchangeable, but through what they notice, want and refuse rather than through vocabulary fences.

R15-2-NO_REGISTER_EDIT  [Pack Fifteen §2]  live
  Register is never corrected during editing and alternate less-modern phrasings are never offered.

R15-2-ONE_TEST  [Pack Fifteen §2]  live
  A word is flagged only if it punctures belief in the world, and the flag goes in author notes with the draft left untouched.

R14-3-STATS_DECIDE_TABLE  [Pack Fourteen §3]  live
  Names which stat answers each Table Rule 5 adjudication question (who bends the room, who closes measure first, whether a working beats armour, whether a read lands, duration and pushing a working, draw cost, wound behaviour, resistance to a hostile working, sustained technique vs refusal, and what a Path gate forecloses), so adjudication is reconstructible from named stats.

R14-3-TRACEABILITY  [Pack Fourteen §3]  live
  Every outcome in a fight must trace to a row in the §3 table, and the author notes must say which.

R14-4-EFFECTS_CHANNEL  [Pack Fourteen §4]  live
  Every stat that decided an outcome must show on the page as behaviour and physics; grade letters and stat names never appear in narration, only their consequences.

R14-6-CHECK28  [Pack Fourteen §6]  live
  Any Grade letter, Stage name, Band, eta, AU/s, EU figure, or Sub-Stat name outside quotation marks, italics, or a marked document block fails.

R14-8-COMBAT_GUIDE_ADJUDICATION  [Pack Fourteen §8]  live
  §3's stat-to-question table becomes the adjudication chapter of the Combat Craft Guide (third edition).

R13-2-CATEGORY_ACROSS_STRATA  [Pack Thirteen §2]  live
  The twenty-six Categories say what kind of thing came out; their alignment tag names which Plane carries the cost and which the counterplay; Category is workbook/Codex-line by default and may be named in diagnostic voice to tell the reader where to hit.

R13-2-CRAFT_DEFINITION  [Pack Thirteen §2]  live
  The Craft (per Pack Ten) is which mouth a working came out of — speaking, cutting, pouring, writing — and it sets the durability of a projected form.

R13-2-STRATUM_ONE_AETHER  [Pack Thirteen §2]  live
  The Aether stratum is what was already in the room before a draw: Aetheric Density (AU/s ceiling), Residue, Saturation, and in the Draw Age the district main/standpipe/meter; its real-science analogue is field, gradient, reservoir, saturation, conduction, and a working changes with local density.

R13-2-STRATUM_THREE_ESSENCE  [Pack Thirteen §2]  live
  The Essence stratum is the soul's water: Soul Crystal architecture (Essence Core, Aether Shell, Attraction Layer) and cost (EU, eta, Flux Density, Crystal State); real-science analogue is efficiency, impedance, resonance, fatigue, phase change of the vessel.

R13-2-STRATUM_TWO_WELLSPRING  [Pack Thirteen §2]  live
  The Wellspring stratum is the law: each of the Sixty carries a Core Law, Family and Physics Domain assigned as real physics, and its law is stated as that real law with a directional operator; the glyph constrains degrees of freedom rather than creating the effect.

R13-2-THREE_STRATA_MANDATE  [Pack Thirteen §2]  live
  Every working explained on the page is explained at three strata (the Aether, the Wellspring, the Essence), in the Technical Register, through one of Pack Twelve's four voices; the strata's causal order need not be the order on the page.

R13-3-PHENOMENON_BANK  [Pack Thirteen §3]  live
  One originated technique seed per Physics Domain (Exuroth, Orrenthal, Absolution, Eclipseron, Fixatio, Mortalis, Somnalis, Vantabriel), each with Wellspring/Category assigned after the phenomenon per the Ability Guide; all eight are pending ratification (see R13-E).

R13-3-PHENOMENON_MANDATE  [Pack Thirteen §3]  live
  Before shipping, every new working/technique/ability names in the author notes the real phenomenon it runs on, the Physics Domain and Wellspring it specialises, the Category and alignment tag, the Mechanism Vocabulary term it dramatises, and the fault the mechanism creates (which must fall out of the phenomenon, not from stamina or convenience).

R13-4-COMBAT_FLOOR  [Pack Thirteen §4]  live
  Every meaningful exchange states, in one of the four voices: measure, tempo (Vor/Nach/Indes), the named read, the fault it exposes, the mechanics of the hit, the injury by structure with ATLS-class blood loss, the Essence account at all three strata, and what the character cannot do next.

R13-4-DENSITY_BUDGET  [Pack Thirteen §4]  live
  A 2,500-word duel carries the full eight-item floor at first display and finisher, plus items 1-4 at every fight-changing exchange; a 700-1,500-word turn carries items 1-4 once and item 8 at close.

R13-4-THREE_EXPLANATIONS  [Pack Thirteen §4]  live
  At first display and finisher, physics, Essence and Hermetic correspondence must each appear as a causal sentence beside its image, not as the image alone.

R13-5-GAP_FILL_PASS  [Pack Thirteen §5]  live
  Before a submitted fight or working is polished, an eight-step gap-fill pass runs (beat inventory, logic audit, stratum audit, physics audit, body audit, phenomenon audit, vocabulary pass, prose pass last), logged in the author notes under "Added by the gap-fill pass."

R13-6-ANATOMY_VOCAB  [Pack Thirteen §6]  live
  Anatomy is named by structure, fractures by fracture type, and neurological consequence by name.

R13-6-CHEMISTRY_BAN_SCOPE  [Pack Thirteen §6]  live
  Pack Six's chemistry ban survives, but only for ambient sensory impression; it does not touch a hit, a wound, or a working.

R13-6-HEMA_VOCAB  [Pack Thirteen §6]  live
  Combat vocabulary draws on the full HEMA range (spear/staff, poleaxe, dagger, grappling, messer, sword and buckler, mounted lance) with a percussion master-strike table built for the hammer specifically.

R13-6-METAPHYSICS_VOCAB  [Pack Thirteen §6]  live
  Correspondence, sympathy and contagion, essence and accident, form and actualisation, and recognition and refusal are the vocabulary of stratum two's Attraction-side reads and of the document voice.

R13-6-REAL_SCIENCE_VOCAB  [Pack Thirteen §6]  live
  Real scientific terms (pressure differential, gradient, latent heat, phase transition, dielectric breakdown, resonance, cavitation, nucleation, information entropy, boundary condition, and the rest) are the vocabulary used to explain what a WOTR noun is doing.

R13-6-SCALING_VOCAB_RESTRICTED  [Pack Thirteen §6]  live
  Scaling-comparison vocabulary (Attack Potency, durability, hax as a concept, speed blitz, reaction vs combat speed, striking vs lifting strength, range, outlier, anti-feat) is used only in author notes and adjudication, never on the page, to keep Natalie honest about who wins and why.

R13-6-WOTR_FIRST  [Pack Thirteen §6]  live
  The glossary and Codex's own nouns (the Planes, Aether, Essence, the Soul Crystal and its layers, Crystal States, Attraction/Obsession Force, the Sixty, the Families, the Categories, the Mechanism Vocabulary, EU/Flux Density/AU-s/eta/Band/Grade/Stage) are used in preference to anything borrowed.

R13-7-RESEARCH_RULE  [Pack Thirteen §7]  live
  Before any set piece, first display, finisher or new working, Natalie searches at minimum once each for the real phenomenon, the real weapon/injury/physiology, and the nearest anime or CRP precedent for the shape, consulting the Codex Spell Index and Magical Categories page, and names what was consulted in the author notes; a set piece written from memory is a failed set piece.

R13-8-RECONSTRUCTIBLE_ADJUDICATION  [Pack Thirteen §8]  live
  Table Rule 5's contested-action decisions must be reconstructible from the page; a fight that resolves because the scene needed it to has failed the pack and is rewritten.

R13-9-COMBAT_GUIDE_EDIT  [Pack Thirteen §9]  live
  The Combat Craft Guide (third edition) has its §4 rewritten off this pack's §4, and its §1 extended per §6; named-character guard and cut assignments run in the same edition.

R13-9-ITEM_GUIDE_WEAPON_ENTRY  [Pack Thirteen §9]  live
  Every weapon entry in the Item and Equipment Writing Guide carries mass, length, point of balance, and the armour tier it beats and fails against.

R13-9-NOTION_PHYSICS_LINE  [Pack Thirteen §9]  live
  The Notion Magical Categories page and Lexicon page each gain a "Physics Domain and real phenomenon" line per entry, giving §3 a wiki home.

R13-9-STYLE_DIRECTIVE_MECHANISM_VOCAB  [Pack Thirteen §9]  live
  Bare jargon mid-action stays banned, but a Mechanism Vocabulary term may now be named in diagnostic voice alongside its dramatisation.

R13-9-VERIFY_CHECKS_22_26  [Pack Thirteen §9]  live
  wotr_verify.sh gains a --combat flag adding checks 22 (HEMA density, warn), 23 (anatomical density, warn), 24 (causal-connective density in fight passages, fail), 25 (a stated fault per named technique, fail), and 26 (a reserve account per working, fail).

R12-1-APPARATUS_CAP_STRUCK  [Pack Twelve §1]  live
  Pack Seven's inheritance of the Apparatus Rule's one-read-per-scene cap is struck and replaced at §4.

R12-1-DESIGN_CHAIN_RETURNS  [Pack Twelve §1]  live
  Pack Seven's confinement of the Design Chain to the workbook is struck; the chain returns to the page at §3.

R12-1-EXPLAIN_BAN_STRUCK  [Pack Twelve §1]  live
  Pack Seven's ban on any sentence explaining why a working produced its effect is struck.

R12-1-MECHANISM_NOWHERE_STRUCK  [Pack Twelve §1]  live
  Pack Seven's "mechanism nowhere" rule is struck.

R12-1-NUMBER_BAN_STRUCK  [Pack Twelve §1]  live
  Pack Seven's ban on numbers attached to metaphysical quantities is struck, subject to the rationing rule at §5.

R12-1-PACK_SEVEN_REPEALED  [Pack Twelve §1]  live
  Pack Seven, the Soft Magic Amendment, is repealed in full; six of its specific provisions are individually struck (see the other §1 rows), and where Twelve conflicts with Seven, Seven is struck.

R12-1-PACK_SEVEN_SURVIVORS  [Pack Twelve §1]  live
  The power economy remains a hard rail (finite, spent, shown on the page), the ladder remains real, and explaining a mechanism does not license a free technique; a fully explained technique that costs nothing is worse than an unexplained one.

R12-1-SOFT_ZONES_STRUCK  [Pack Twelve §1]  live
  Pack Seven's three soft zones as a blanket rule are struck, with one exception flagged at §6.

R12-2-MYSTIC_REGISTER_DEF  [Pack Twelve §2]  live
  The Mystic Register governs Wellsprings, sites, rites, oaths, the Veil, and anything the world does regardless of who is watching; it is stated as law, never as physics — imperative rules, prices given as prices, taboos given without justification.

R12-2-NO_MIXING  [Pack Twelve §2]  live
  A passage that mixes the Technical and Mystic registers reads as a textbook and is the pack's primary failure mode.

R12-2-REGISTER_TEST  [Pack Twelve §2]  live
  If a person chose to do a thing, it is Technical Register. If it was already running before anyone noticed, it is Mystic Register.

R12-2-TECHNICAL_REGISTER_DEF  [Pack Twelve §2]  live
  The Technical Register governs combat, craft, injury and anything a practitioner does on purpose; exhaustive, tactical, delivered in-fight, explaining the read, the fault, the counter, the reserve and why the answer worked, in real vocabulary.

R12-3-COMBAT_EXCHANGE_OWES  [Pack Twelve §3]  live
  Across an engagement, at minimum: the read (evidence named), the fault (structural weakness), the counter and why it works (causal), the cost in body and reserve (both), and what the character cannot do next.

R12-3-COUNTER_MANDATORY  [Pack Twelve §3]  live
  Ability entries retain the Counter field as mandatory; explanation makes counterplay easier to write, not optional.

R12-3-DESIGN_CHAIN_RETURNS  [Pack Twelve §3]  live
  The Design Chain (Trigger, Function, Mechanism, Numerical Effect, Target Response, Consequence, Limitation, Weakness, Cost, Counterplay) is fully prose-legal, distributed through the scene rather than workbook-only.

R12-3-NAMED_INVENTOR_RULE  [Pack Twelve §3]  live
  Every technique was derived by somebody for a specific problem; a documented technique is counterable by anyone who studied it, while a self-derived technique must be read live in the exchanges before it kills, shown in full rather than summarised.

R12-4-VOICE_DISCIPLINE  [Pack Twelve §4]  live
  A Measurewright, a Kharven hunter and an Accord officer explain the same phenomenon in three different vocabularies; which words a mouth reaches for remains class-marking per Pack Nine.

R12-4-VOICE_FOUR_DOCUMENT  [Pack Twelve §4]  live
  An in-world register entry, field manual, or codex line dropped in at a scene threshold (the LotM model), using the Mystic Register and setting a rule the scene then breaks.

R12-4-VOICE_MIXING_RULE  [Pack Twelve §4]  live
  Confirmed by Isaac's ruling: the four explaining voices may mix, whoever is in the scene, picked per scene, with no more than two run in one engagement.

R12-4-VOICE_ONE_POV  [Pack Twelve §4]  live
  Free indirect explanation ceilinged at the POV character's actual competence; an out-of-depth character explains wrong, and a confidently wrong technical explanation is now the primary vehicle for the misreading budget.

R12-4-VOICE_THREE_OPPONENT  [Pack Twelve §4]  live
  A practitioner explains exactly what he is doing to his target while doing it, because he does not believe the explanation will help; the most economical and characterful voice, barred to anyone without standing to be arrogant.

R12-4-VOICE_TWO_SECOND  [Pack Twelve §4]  live
  A present observer who understands what the POV does not, narrating the read to a third party or himself (the Kakashi model); requires an actual reason to be present and talking, best carried by teachers, rivals, Measurewrights, or the Research and Archives Division.

R12-5-NUMBERS_DIAGNOSTIC_ONLY  [Pack Twelve §5]  live
  EU, AU/s, eta, Coherence, Grade and Band numbers may not reach third-person narration on the narration's own authority; they are legal only in diagnostic voice (a faculty reading, a practitioner counting reserve, a document, an instrument, or a speaking character).

R12-6-MYSTIC_GRAMMAR  [Pack Twelve §6]  live
  The Mystic Register's grammar: imperative without justification, price stated exactly with no reason given, correspondence in place of causation, attribution to an uncertain institution, and the recorded failure documented in flat administrative prose.

R12-6-RITUAL_SPECIFICATION  [Pack Twelve §6]  live
  A rite has a fully specified bill of goods, an order of operations, and a list of ways it goes wrong.

R12-6-RULES_AS_RULES  [Pack Twelve §6]  live
  The Mystic Register states consequences without offering mechanism (the canonical example: do not carry Object Wellsprings into Site Wellsprings, no mechanism given, a stated consequence, the Guild has buried those who tested it); more sentences are written in that shape.

R12-7-AI_TELLS_RETAINED  [Pack Twelve §7]  live
  The Ladder ban, the Gloss budget of zero, local burstiness, the reification budget, and every AI tell in the checklist survive unchanged; explaining a mechanism is no licence to write badly.

R12-7-APPARATUS_CAP_REPLACED  [Pack Twelve §7]  live
  Pack Six's Apparatus Rule one-read-per-scene cap is struck; a faculty may now read as often as the scene needs, replaced by a cost — a read occupies attention, and the scene shows what the reading cost the reader in tempo.

R12-7-IGNORANCE_MISREADING_RETAINED  [Pack Twelve §7]  live
  The ignorance quota and misreading budget survive Pack Twelve intact and become more important as dramatic-irony tools now that explanation is the default; the wrong explanation is now the load-bearing device.

R12-7-LEGIBILITY_BAN_STRUCK  [Pack Twelve §7]  live
  Pack Six's total-legibility ban and blanket interpretation-at-zero rule are struck.

R12-8-COMBAT_GUIDE_READ_LENGTH  [Pack Twelve §8]  live
  In the Combat Craft Guide, the read is now shown at length rather than summarised, and the three-layer hit model is rendered as explanation, not only as phenomena.

R12-8-SCENE_GUIDE_PACK7_STRUCK  [Pack Twelve §8]  live
  The Scene Writing Process Guide's Pack Seven pre-write question and self-review check are struck.

R12-8-SCENE_STANDARDS_BANS_STRUCK  [Pack Twelve §8]  live
  Scene Standards' sentence-level bans on mechanism explanation and metaphysical numbers are struck; everything else in Scene Standards stands.

R11-1-REFERENCE_TRIANGLE  [Pack Eleven §1]  live
  Warhammer 40,000 for bureaucratic scale, late-Victorian industrial imperialism for material texture and class geography, and manhwa for visual power-display grammar — the last now widened from Pack Nine's four beat types to the general visual register. References are not to be imitated directly.

R11-2-APPARATUS_INHERITS_PHYSICS  [Pack Eleven §2]  live
  A machine is a conduit retuned to a current (phase-locking, not restricted to souls); every apparatus inherits its Family's favoured condition, suppressive condition, and characteristic pathology off the Environmental Coupling table.

R11-2-HYDROLOGY_MODEL  [Pack Eleven §2]  live
  The technological age runs on Wellsprings as standing currents that pool where substrate/competing-currents/time allow, per The Mechanism of the Sixty; nobody built an engine, they sank a well.

R11-2-MEASUREWRIGHTS  [Pack Eleven §2]  live
  The Guild of Measurewrights (Hexagonal Oath, in concurrence with Research & Archives) wrote the address down but does not explain the mechanism.

R11-3-AMMO_TIERS  [Pack Eleven §3]  live
  Common shot (cheap, kills men, annoys practitioners), proofed shot (Materia-coupled, Guild-stamped, defeats proofed plate but not a Domain), and the named round (one-shot assassination ordnance, Wellspring-coupled and paid for in advance).

R11-3-ARMOUR_STOPS_SHRINKING  [Pack Eleven §3]  live
  Shot is chemically cheap and does not care about a soul; a working is not shot, so mass and material still buy a fraction of a second against a Wellspring-carried strike, which is why armour retreats to the cuirass-and-helm silhouette and stops there.

R11-3-BALLISTICS_RULING  [Pack Eleven §3]  live
  Powder weapons are slow, loud and honest; a practitioner above roughly the middle Stages is not reliably killed by common shot, which is why the enchanted round exists at the cost it does.

R11-3-DUAL_ANSWER_SILHOUETTE  [Pack Eleven §3]  live
  The silhouette is coat, breast, helm, sword and firearm; every armed man carries at least two answers because there are at least two questions, which is why the Combat Craft Guide survives intact — Liechtenauer and Fiore still govern inside twenty feet.

R11-3-FIREARM_PROSE_LAW  [Pack Eleven §3]  live
  The gun is furniture, not spectacle; no sentence explains why a proofed round defeats proofed plate — the physical account of the hole stays, the causal account is cut.

R11-3-GUNPOWDER_AND_ARMOUR_CONFIRMED  [Pack Eleven §3]  live
  Gunpowder exists, magical weapons exist, and armour is not dead.

R11-4-DEPTH_AXIS  [Pack Eleven §4]  live
  A Well is not a hole in the ground; it is a place where the surface (Physical) has thinned enough that the middle layer (Soul) reaches through toward Fate, the deepest plane.

R11-4-DESCENT_PROSE_LAW  [Pack Eleven §4]  live
  A descent's sensory opening favours the body (feet, teeth, pack weight) over the room; the ignorance quota becomes the ambient condition rather than a per-scene quota, and a misidentified encounter type is the best-shaped disaster the setting offers.

R11-4-MAGNITUDE_AXIS  [Pack Eleven §4]  live
  Magnitude runs Ambient through Core with attested rungs (Ambient Saturation, Active Concentration, Veil-Thin, Core); intermediate rungs must be pulled from source before use and never invented.

R11-4-WELLS_ORIGIN  [Pack Eleven §4]  live
  At sufficient concentration, a law the landscape has been measured by for so long that the landscape has become the law's argument, per The Mechanism of the Sixty.

R11-5-COMBAT_GUIDE_RANGE_OPENING  [Pack Eleven §5]  live
  The Combat Craft Guide is unchanged in substance, with the addition that an engagement may open at range and the HEMA grammar takes over inside roughly twenty feet.

R10-1-CATEGORIES_AXIS  [Pack Ten PART ONE]  live
  The Magical Categories (Arts, Drafts, Conjunction, Synergia, Harmonia, Ritus, Phenomena, Glyphica, Manifestus, Viaforma, Traitus, Mechanica, Crystalia, Wellsprings, Animatria, Vocatia, Vectra, Mnemata, Aetherica, Concordia, Domain Weaving, Claustra, Silentia, Auguria, Transitus, Votia) are ratified as live canon, answering what kind of thing a working is, alongside the Four Crafts' answer of how it is performed.

R10-1-CRAFT_VS_CATEGORY  [Pack Ten PART ONE]  live
  Craft and Category cross like Family and Method at a coarser grain — Craft is the mouth a working comes out of, Category is the shape of the thing that comes out; the same category can be delivered by different crafts with different results.

R10-2-CATEGORY_READ_EXCEPTION  [Pack Ten PART TWO]  live
  A scene's single diagnostic read (the Apparatus Rule's existing budget, not a new one) may be spent naming the category instead of a Wellspring or Family; naming a category doesn't trip Check 20, but it still costs the scene's only read.

R10-2-CATEGORY_REGISTER_DEFAULT  [Pack Ten PART TWO]  live
  Category names sit at Wellspring/Family register, not common tongue, and stay workbook and Codex-line by default.

R10-3-ANIMATRIA_SHAPE  [Pack Ten PART THREE]  live
  An Animatria shape moves with the caster's active attention, can obey or strain against instruction, and stops being anything if the live line back to its maker is severed (Silence, a killed caster, a broken focus).

R10-3-CRAFT_DURABILITY_TABLE  [Pack Ten PART THREE]  live
  Spellcraft holds only while attention holds; Runecraft is fixed and degrades like any inscription; Draftcraft is bottled, cruder, and buyer-triggerable; Chantcraft requires the sustaining choir and is pending Chantcraft's own ruling.

R10-3-EFFECT_PHYSICAL_ONLY  [Pack Ten PART THREE]  live
  A projected shape's Effect describes only what it looks/does/feels like, never why the Wellspring resolved as that particular shape; if that answer exists at all, it stays workbook, one line, never on the page.

R10-3-MANIFESTUS_SHAPE  [Pack Ten PART THREE]  live
  A Manifestus shape is involuntary, arriving on a pressure or emotion spike, short-lived by nature, and reads as a Trait bleeding through rather than a tool being used; nobody aimed it.

R10-3-PROJECTED_FORM_LICENSE  [Pack Ten PART THREE]  live
  Ability Effect fields may render a working as an externalised shape (animal, elemental figure, guardian silhouette) instead of only as discharge, pressure, or weather.

R10-3-TRAITUS_SHAPE  [Pack Ten PART THREE]  live
  A Traitus shape is passive and permanent, not fired or summoned but a standing mark of what the Crystal has become, visible only under a specific condition and gone the instant that condition lapses.

R9-1-FOUR_CRAFTS_RATIFIED  [Pack Nine PART ONE]  live
  Magicraft, Spellcraft, Runecraft and Draftcraft are confirmed canon as the naming scheme for how a working is performed; Chantcraft stays pending.

R9-2-COMMON_TONGUE_LEGAL  [Pack Nine PART TWO]  live
  A POV may be shown cutting a ward, speaking a working, or pouring a Draft (singing pending Chantcraft's ruling); the common tongue for craft actions is prose-legal, said the way the character would actually say it, while the Accord Latin terms stay document-only.

R9-2-UNTOUCHED_BANS  [Pack Nine PART TWO]  live
  The mechanism gloss ban (Check 20), the metaphysical-units ban (Check 21), the chemistry ban, and the physical-Effect rule are untouched — a line may say a man cut a ward into a doorframe but may not say why the cutting suppressed the Essence.

R9-2-VOCAB_BAN_NARROWED  [Pack Nine PART TWO]  live
  Pack Seven's ban on system vocabulary in prose still binds absolutely against Wellspring names, Family, Physics Domain, Stage/Grade, EU, other metaphysical units, and Codex glyph designations; it no longer binds against the craft register.

R9-3-AURA_AS_FRACTURE  [Pack Nine PART THREE]  live
  Essence discharge reads as broken, shard-edged geometry around the body rather than a soft glow; "it glowed" stays banned, replaced for Stage-display beats specifically, not as a general phenomenon substitute.

R9-3-BODY_UNDER_LOAD  [Pack Nine PART THREE]  live
  A Pressure spike or peak-effort feat renders as visible physical distortion, ink-dense like a splash panel; WOTR is licensed exactly one splash-panel beat per scene, at the display, never twice.

R9-3-COST_UNCHANGED  [Pack Nine PART THREE]  live
  The ignorance quota, the misreading budget, and the one-read Apparatus Rule bind a Stage-display beat exactly as hard as any other scene.

R9-3-EYES_AS_TELL  [Pack Nine PART THREE]  live
  At a Stage threshold or dropped suppression, the eyes go first — light behind them, overtaking iris and sclera — rendered as a fact the room reacts to, still costing the Apparatus Rule's one read.

R9-3-SCOPE  [Pack Nine PART THREE]  live
  The new power-display register applies only to Stage-ascension beats, a Pressure drop, and a technique's first display or finisher; the Visual Aesthetic Guide's grounded North Star governs everywhere else (wounds, environment, objects, ordinary combat, material culture).

R8-11-SHEET_EXPLAINS  [Pack Eight 1.1]  live
  A character sheet, codex entry or technique writeup states the literal operation in plain functional language, with no atmosphere or withholding, so a reader can adjudicate a fight from the sheet alone.

R8-12-SHEET_GETS_OPERATIONAL_ACCOUNT  [Pack Eight 1.2]  live
  Pack Seven's rule that the page gets only Effect/Cost/Limit/Counter and everything else stays workbook is amended: the Design Chain remains workbook, but the operational account of how an ability functions becomes sheet content.

R8-14-CLASSIFICATION_BLOCK  [Pack Eight 1.4]  live
  Class (Offensive/Defensive/Supplementary), Family (one of the Eight), Wellspring (from the Sixty, primary first), Method (one of eight named methods), Physics Domain (one of the Nine), Grade (fixed Tier Grade scale), Range (real units), and Medium — all drawn from live WOTR taxonomy, nothing invented.

R8-15-DESCRIPTION_STYLE  [Pack Eight 1.5]  live
  The description is written the way a man teaching it would write it: flat operational prose, no mood, no cadence work, no elegiac register — that belongs in scenes.

R8-24-RELEASE_MECHANIC  [Pack Eight 2.4]  live
  An art with a true name may carry a release call (imperative verb plus name); it is never required for the art to function, costs a beat for a measurable output increase, and is spoken indistinguishably by a man in real danger or a man showing off.

R8-26-GLOSS_DEPENDENCE_FAILS  [Pack Eight 2.6]  live
  A working whose meaning must be glossed for the scene to land is a working that failed; fix the working, not the gloss.

R8-26-GLOSS_NEVER_IN_PROSE  [Pack Eight 2.6]  live
  The gloss never appears in prose, with no translation apposition; the reader gets it from the sheet or from another character explaining it in dialogue for their own reason.

R8-26-NAME_NEVER_NARRATED  [Pack Eight 2.6]  live
  The true name is spoken by a character; it never appears in narration.

R8-26-ONE_RELEASE_PER_SCENE  [Pack Eight 2.6]  live
  One release per scene at most; two men releasing in the same scene is an event, not texture.

R7-1-THREE_HARD_RAILS  [Pack Seven PART ONE]  live
  Three things never bend: the economy (finite, spent, visible), the ladder (sixteen Stages, felt before seen), and the lineage (everything traceable to a source).

R6-1-LADDER_BAN  [Pack Six PART I.1]  live
  Two or more clauses of the form "X was Y and Y was Z" in one sentence is an automatic cut, never a rewrite; cut back to the first concrete noun. Restating a concrete observation as an abstraction in the following sentence is barred the same way.

R6-11-CHECK18_LADDER  [Pack Six §11]  live
  Flags any sentence containing two or more instances of the "was X and the X was" pattern, including is/became/meant variants.

R6-11-CHECK19_APPARATUS_SUBJECT  [Pack Six §11]  live
  Flags any sentence where a named faculty is the grammatical subject of a perception verb; high false-positive rate by design, reviewed by reading.

R6-12-TELL_BANK_SURVIVING  [Pack Six §12]  live
  Added to the AI tells checklist and retained in full per Pack Twelve §7 — the Ladder in all forms, the apparatus as grammatical subject, scientific vocabulary explaining an ambient sensory impression, and fresh texture invented where Inventory texture exists.

R6-2-CHEMISTRY_BAN  [Pack Six PART I.2]  live
  Real-world scientific vocabulary explaining a sensory impression is barred unless the POV is a practitioner of that specific craft actively diagnosing; the Mechanism Standard governs technique trigger points and nothing else, never a licence for ambient exposition.

R6-2-FACULTY_NEVER_SUBJECT  [Pack Six PART I.2]  live
  A named faculty is never the grammatical subject of a perception verb; the character perceives, the apparatus does not act on its own behalf.

R6-2-READ_DELIVERS_FACT  [Pack Six PART I.2]  live
  A faculty's read delivers a bare fact ("Twenty-three"), never an interpretation woven around it; the faculty is an instrument and instruments produce numbers, not meaning.

R6-3-IGNORANCE_QUOTA  [Pack Six PART I.3]  live
  At least one thing per scene the POV notices and cannot interpret; the narration does not rescue him, and resolution comes in a later scene or chapter.

R6-4-MISREADING_BUDGET  [Pack Six PART I.4]  live
  At least one POV inference per scene is wrong and is not corrected on the page; the misreading must be reasonable, consistent with what the POV knows, and left standing for the reader to find later.

R6-5-MATERIAL_DENSITY_SURVIVES  [Pack Six PART I.5]  live
  Material density (rust, weathering, wear, labour) stays described in full per the Directive; the distinction is that the world is described exhaustively but interpreted at zero — meaning and thematic weight are withheld.

R6-7-GOVERNING_RULE  [Pack Six PART II.7]  live
  Texture comes from the Standing Inventory; mechanism comes from the Codex; a scene that reaches for the Codex when it needs texture has failed.

R6-9-RECURRENCE_RULE  [Pack Six PART II.9]  live
  Every scene draws texture from the Inventory before inventing anything; anything newly invented is entered into the Inventory the same session or it does not exist; each culture designates three-to-five high-frequency signature items on the Recurrence Ledger.

R5-A-FLESH_CHANGES  [Pack Five A]  live
  Prose register, humour, characterisation method, narration authority and tonal architecture derive from the Western tradition (Martin's POV discipline, Tolkien's cultural register and elegiac capacity, Abercrombie's dry brutality).

R5-A-OPERATIVE_CONSEQUENCE  [Pack Five A]  live
  The system is author-facing and document-facing — it lives in the Codex, character sheets, in-world treatises and technical-reveal beats; it does not live in prose texture, narrator commentary, or character affect.

R5-A-SKELETON_UNCHANGED  [Pack Five A]  live
  WOTR runs an Eastern skeleton under Western flesh; the systematic density (Stages, Wellsprings, Bands/Grades/Coherence/eta, Codex glyph grammar, Family/Physics Domain, the character sheet, the Design Chain, EU costs) does not get diluted, softened, or made impressionistic.

R5-B-DECLARATION_TO_INFERENCE  [Pack Five B]  live
  A character is established by what they choose under pressure, what they refuse, what they notice first, and what others say about them when absent — not by being vividly themselves in every line; the reader assembles the person.

R5-B-FOIL_FUNCTION_SURVIVES  [Pack Five B]  live
  Deliberate moral and rhythmic contrast between paired characters (Merry/Pippin, Sam/Frodo, Tyrion/Bronn, Jaime/Brienne, Glokta/Severard) is Western-native and remains actively tracked.

R5-B-MANHWA_DIRECTIVE_REPEALED  [Pack Five B]  live
  Personality vomiting, reaction shots as characterisation, loud opinions as a default register, bizarre/exaggerated minor NPCs, absurdist method alongside genuine stakes, humour as a tone exception, and escalation as a prose-level pacing register are no longer craft law and become active tells; escalation survives only as plot architecture (Scene Writing Process Guide §2).

R5-C1-NARRATION_NEVER_WINKS  [Pack Five C.1]  live
  No line acknowledges that the reader knows better than the POV.

R5-C2-FLAT_DELIVERY  [Pack Five C.2]  live
  Section 1.2's saga restraint governs elegy: state the loss and move on, without raising the prose's voice.

R5-D-HUMOUR_BARRED  [Pack Five D]  live
  The beat structure of setup, deadpan and reaction is barred; humour arrives inside dialogue and narration already in progress and never pauses the scene to land.

R5-D-HUMOUR_PERMITTED  [Pack Five D]  live
  Humour is no longer an exception to tone but a property of specific characters in the Abercrombie register: dry understatement, gallows wit from those whose profession earns it, class-inflected contempt, a character being funny without knowing it, and the joke that is also a threat.

R5-E-ELEGIAC_REGISTER_ALLOWED  [Pack Five E]  live
  The elegiac register is available to scene narration because elegy in the saga mode reports a loss rather than judging it; Section C.2's flat-delivery rule is what keeps it on the correct side of the line.

R5-E-FREE_INDIRECT_CARVEOUT  [Pack Five E]  live
  A moral verdict in the POV's own idiom, which could be wrong, is characterisation; the same verdict in the narrator's neutral register is Tolkien's voice and gets cut, per Pack Four's gloss test.

R5-E-MARTIN_WINS_RULING  [Pack Five E]  live
  Martin wins on narration authority: Tolkien's elevated and elegiac register is available, his moral voice is not.

R5-E-NARRATION_NEVER_ADJUDICATES  [Pack Five E]  live
  Third-person scene narration is POV-locked and never adjudicates; no line tells the reader what to think of a character's choice.

R5-F-LIGHT_NOVEL_KEPT  [Pack Five F]  live
  The chapter as a hard unit with one objective and a hook at close; legibility as reader pleasure (the argument for the Codex); and fast entry, chapters opening inside the situation.

R5-F-LIGHT_NOVEL_NOT_KEPT  [Pack Five F]  live
  Status screens, panels and HUD-style rendering (already banned); narrator explanation of the system to the reader; the isekai commentary register (wry narrator asides about the world's rules); and character-sheet vocabulary (Stage, Grade, eta, Band) surfacing in prose.

R5-G-PROJECT_INSTRUCTIONS_EDIT  [Pack Five G]  live
  The Manhwa Energy Directive block is struck in full from project instructions, "manhwa-cinematic" and "manhwa energy" are struck from tone descriptions, and the genre-intersection statement is amended so only the pacing ambition survives.

R5-G-TELL_BANK_ADDITIONS  [Pack Five G]  live
  Declarative characterisation and the reaction-shot cutaway (from Section B's confirmed repeal) plus the comedic beat structure and narrator moral adjudication (from Sections D and E, still proposed) are added as active tells.

R4-13-DELETION_TEST  [Pack Four Amendment Thirteen]  live
  Delete the candidate sentence and read the passage; if it means the same thing, it was a gloss and stays deleted; if meaning is genuinely lost, the failure is in the line above and that line is rebuilt, never the gloss restored.

R4-13-FID_CARVEOUT  [Pack Four Amendment Thirteen]  live
  A sentence in the POV's own idiom that could be wrong and reveals the character interpreting is characterisation and is kept; a sentence in the narrator's neutral, authoritative register that reveals nothing about anybody's interior is cut.

R4-13-GLOSS_DEFINED  [Pack Four Amendment Thirteen]  live
  A gloss is any sentence whose only function is to state the significance of the sentence before it, disguised as texture or observant narration.

R4-13-THIRD_RECURRENCE  [Pack Four Amendment Thirteen]  live
  A loaded word gets one physical response on first use and no comment; meaning arrives on the third appearance through change — another character uses it, the usual user withholds it, or register makes earlier uses retroactively legible. Withholding is the strongest of the three.

R4-13-THREE_DISGUISES  [Pack Four Amendment Thirteen]  live
  The etymological gloss explains why a character chose a word; the register gloss explains what a tone signified; the stakes gloss restates in abstract vocabulary an event the reader already watched concretely, usually at paragraph-end.

R4-13-WHERE_SIGNIFICANCE_LIVES  [Pack Four Amendment Thirteen]  live
  Significance may live in a physical tell adjacent to the line, in another character's bodily reaction, or in a governed callback — never in narration itself.

R4-13-ZERO_BUDGET  [Pack Four Amendment Thirteen]  live
  The gloss budget is zero per scene; there is no acceptable first instance.

R4-14-APPROACH_LADDER  [Pack Four Amendment Fourteen]  live
  Decelerate into the hit (long, then medium, then the landing) so the reader's eye slows as it arrives, then expand again afterward since consequence is cumulative and belongs in cumulative syntax.

R4-14-CHAIN_CEILING  [Pack Four Amendment Fourteen]  live
  No more than two consecutive sentences may run over 25 words; the third breaks, or the paragraph does.

R4-14-CLAUSE_CAP_AT_BEAT  [Pack Four Amendment Fourteen]  live
  Within the three sentences bracketing a value turn, no sentence carries more than one subordinate or coordinate clause.

R4-14-CONJUNCTION_AUDIT  [Pack Four Amendment Fourteen]  live
  Count coordinators per sentence (and, as, while, which, because); two or more inside a beat-carrying sentence is a fail, though the same count outside a beat is style and left alone.

R4-14-HARD_CEILINGS  [Pack Four Amendment Fourteen]  live
  A payoff sentence carries at most 10 words and zero subordinate clauses; at least 18% of a scene's sentences run under 8 words; and no paragraph closes on three consecutive sentences over 18 words.

R4-14-LANDING_RULE  [Pack Four Amendment Fourteen]  live
  The emotional hit takes the shortest sentence in its paragraph and sits last; anything trailing it is the paragraph clearing its throat after the point.

R4-14-PARAGRAPH_VARIANCE  [Pack Four Amendment Fourteen]  live
  Sentence-length variance is enforced at the paragraph level, not just the document level; the global CV stays as a floor and stops being sufficient on its own.

R4-14-RUN_RULE  [Pack Four Amendment Fourteen]  live
  No three consecutive sentences may fall within forty percent of each other's word count.

R4-15-BODIES_EXEMPT  [Pack Four Amendment Fifteen]  live
  Concrete nouns doing concrete things (hands shaking, a jaw setting, a tail going still) are not counted against the reification budget.

R4-15-CLERK_TEST  [Pack Four Amendment Fifteen]  live
  If the thing could appear as a line on a ledger, in a writ, or in a Bench finding, reify it freely; if it could not, it costs budget.

R4-15-CONCRETE_FIRST  [Pack Four Amendment Fifteen]  live
  Before reifying, check whether an object already in the room can carry the same freight; the object always wins.

R4-15-DENSITY_PROBLEM  [Pack Four Amendment Fifteen]  live
  Reification is a genuine Hermetic-register instrument and any single instance reads well; the failure is density — ten good instances in a scene leave nothing solid for the reader to stand on.

R4-15-LICENSED_CLASS  [Pack Four Amendment Fifteen]  live
  A debt, claim, verdict, name, standing, bearing, holding, writ, seal, title, warrant, attribution or tally is a closed-class WOTR abstraction the world already treats as a thing that moves between hands; reifying these is description, not device, and is unbudgeted and free.

R4-15-NEVER_AT_BEAT  [Pack Four Amendment Fifteen]  live
  Reification is atmosphere and a poor delivery vehicle for a turn; the value flips on a body or an object, never on a personified noun.

R4-15-ONE_PER_PARAGRAPH  [Pack Four Amendment Fifteen]  live
  Two unlicensed reifications in a single paragraph produces a bogged-down feeling independent of the scene total.

R4-15-PROCEDURAL_SCENE_BUDGET  [Pack Four Amendment Fifteen]  live
  In procedural or ledger scenes, the unlicensed reification budget tightens to one per page, since the Licensed Class is already doing that work there and unlicensed instances would go invisible against it.

R4-15-REIFICATION_DEFINED  [Pack Four Amendment Fifteen]  live
  Reification gives an abstract noun a physical verb, handled by characters as an object in the room; the tell is the verb, not the noun (hand, pass, set, carry, place, take, give, hold, put, return, offer, lift, drop, weigh, lay, arrive, land, sit, settle, drain).

R4-15-TWO_PER_SCENE_BUDGET  [Pack Four Amendment Fifteen]  live
  At most two unlicensed reifications per scene, of different abstractions; a third is a fail regardless of quality, and the same abstraction may be reified only once.

R4-15-WATCHLIST_NOUNS  [Pack Four Amendment Fifteen]  live
  Recurring reification offenders, kept greppable rather than felt: silence, weight, arithmetic, governance, permission, distance, authority, refusal, grief, history, patience, the question, the cost, the moment, the space between them.

R4-ADD-CHECK15  [Pack Four Addendum]  live
  Flags etymological, register and "which meant" constructions; warns rather than fails since the whose-vocabulary test cannot be automated, and every flag gets read.

R4-ADD-CHECK16  [Pack Four Addendum]  live
  Fails any chain of 3+ sentences over 25 words, 4+ flat runs, a short-sentence share under 10% (warns under 18%), and reports every paragraph closing long-long-long.

R4-ADD-CHECK17  [Pack Four Addendum]  live
  Fails at 3+ unlicensed reification instances or any paragraph carrying two; licensed instruments are detected, reported separately, and never counted against budget.

R4-H1-WREN_ASSIGNED  [Pack Four Still Open]  live
  Blade grammar; patient under load, gives ground on the shield rather than trade for it, then closes into Nachreisen (a German longsword-tradition term: "following after" — chasing the opening that appears right after a block or parry, instead of attacking on your own initiative) with the arming sword; does not fight the armor, reads for the one unarmored inch and puts the point there.

R4-H2-EDWARD_LAMBERT_ASSIGNED  [Pack Four Still Open]  live
  Blade grammar; half-swording (Halbschwert — gripping partway up his own blade with the off hand for extra leverage and control on a thrust) up under the jaw; turns the point with the hip rather than the wrist because at fifty-three the wrist goes first; no exchanges, one finish.

R3-7-COZBI_ASSIGNED  [Pack Three Amendment Seven]  live
  Grammar Expenditure; signature move the Standing into the Ledger, not a bluff; answers plate with indifference; tells by crouching to examine rather than posturing, with speeded cadence signalling genuine alarm; status canonical, transcribed from the character card.

R3-7-COZBI_CRAFT_NOTE  [Pack Three Amendment Seven]  live
  An Expenditure fighter is boring to write badly and terrifying to write correctly; refuse the temptation to give him a spectacular exchange — the scene should feel like the opponent is winning, repeatedly, at increasing cost, until the arithmetic arrives.

R3-7-EXPENDITURE_ASSIGNMENT  [Pack Three Amendment Seven]  live
  Assigned to Cozbi, Legion summoners, construct-users, anyone operating through a conscripted lattice or projected vessel, and any fighter whose combat proposition is that the target of the blow is not the source of the working.

R3-7-EXPENDITURE_GRAMMAR  [Pack Three Amendment Seven]  live
  Expenditure grammar does not seek to defeat the opponent; it makes the exchange asymmetric so everything the opponent does is expensive and everything the practitioner does is spent from somewhere else. Victory is arithmetic and arrives late.

R3-7-EXPENDITURE_MOVES  [Pack Three Amendment Seven]  live
  The Offering converts opponent capability into expenditure; the Standing converts time into advantage (only for a fighter who doesn't experience time as a resource); the Ledger converts the whole preceding engagement into one effect, un-repeatable per encounter; the Absence converts a victory into nothing by destroying the vessel while the practitioner is unaffected.

R3-7-EXPENDITURE_TELL_AND_COUNTERPLAY  [Pack Three Amendment Seven]  live
  The tell is that the fighter is not looking at the opponent. Counterplay is reaching through the projection to the claims it carries (a Rescind-type working) or forcing a direct physical contest against the practitioner's own below-ceiling frame; both are documented as having worked once, neither is easy.

R3-7-EXPENDITURE_VS_PLATE  [Pack Three Amendment Seven]  live
  Plate is a cost problem rather than a defence problem to an Expenditure fighter, who is content to let armour do its job for as long as the wearer can afford to keep wearing it.

R3-8-CAVALRY_PURSUIT_FUNCTION  [Pack Three Amendment Eight §14]  live
  Per Section 6, pursuit is butchery of the unresisting from behind at a canter for as long as the horses last; the honest version does not let the cavalry POV enjoy it and does not let them refuse it either.

R3-8-NAVAL_FIRE_WATER_DEATHS  [Pack Three Amendment Eight §12]  live
  Naval death is written as fire and water, not the blade.

R3-8-NAVAL_MOTION_CHANNEL  [Pack Three Amendment Eight §12]  live
  Every physical action on a moving deck costs balance; fighting on a moving surface is closer to fighting drunk than to fighting on a slope and should be written that way.

R3-8-SIEGE_TEDIUM_BY_SPECIFICITY  [Pack Three Amendment Eight §13]  live
  One precisely observed repeated detail carries a month — the same gap in the wall, seen from the same place, on a different day, with something small changed.

R2-5-RESOLUTION_IN_AFTERMATH  [Pack Two Amendment Five]  live
  The relationship change is legible in how the two characters handle an unrelated later scene; if a character ever says what the act meant, the amendment has been violated and the scene has been spent.

R2-5-SINGLE_ACT_CONDITIONS  [Pack Two Amendment Five]  live
  The Single Act must be physical (a thing done with the body under load, not a decision or realisation), unrepeatable (the circumstance does not recur), witnessed but not discussed during the engagement, and ambiguous at the moment it happens to witness, reader and actor alike.

R1-1-BLACK_AGENT_ASSIGNED  [Pack One Amendment One]  live
  Blade grammar; unhurried, every word and motion in exact position, Absetzen and Durchwechseln rather than committed cuts; does not engage plate, waits.

R1-1-BLADE_GRAMMAR  [Pack One Amendment One]  live
  Applies to characters trained in a conventional sword tradition (guards, master-cuts, the bind, Winden, Indes, Absetzen, Durchwechseln); the full existing framework applies without change, assigned to conventional duellists, Concord Aether Knights, Old World nobility, and guild fencing traditions.

R1-1-DOUGOU_ASSIGNED  [Pack One Amendment One]  live
  Percussion grammar; signature the Settling into the Iron Step, striking the ground before the body; answers plate with Black-Heart Bison Break, backforce rather than penetration; canonical.

R1-1-PERCUSSION_MOVES  [Pack One Amendment One]  live
  The Settling breaks stance (descending full weight into the base); the Sweep breaks the base (horizontal, full arc and mass); the Shove breaks measure (haft/shoulder/boss/pommel at closed distance); the Return breaks recovery (the rebound redirected into a second strike) and is the grammar's signature.

R1-1-PERCUSSION_VS_BLADE  [Pack One Amendment One]  live
  A swordsman asks where you are unguarded; a hammer asks where you are already carrying load — fatigue accumulates and openings do not, so percussion fights get more dangerous the longer they run while blade fights get less decisive.

R1-1-PERCUSSION_VS_PLATE_AND_TELL  [Pack One Amendment One]  live
  Percussion does not seek to penetrate plate, it uses it — armour transmits force inward, so the target is the joint, base and balance, making the plate expensive to wear. The tell is that the fighter tracks breathing and footing rather than the weapon.

R1-1-RASHANI_ASSIGNED  [Pack One Amendment One]  live
  Verdict grammar; signature Origin read then Zero-Point deflection, wins by knowing rather than reaching; answers plate with a structural read through to what it's bolted to; canonical.

R1-1-RENGAI_ASSIGNED  [Pack One Amendment One]  live
  Percussion grammar; signature the Return, narrowing post-injury into economy rather than flourish; answers plate by using it (joints and base); the post-injury reading follows from the Cinder Lung material and needs confirmation.

R1-1-RESTRICTED_CHARACTERS_EXCLUDED  [Pack One Amendment One]  live
  Restricted characters (Ma'Kovu, Fushigi, Haruki, Xhem, Dova'Kan, Gorgi) are excluded entirely from this combat-vocabulary framework; their assignments belong to their creators.

R1-1-SODOKU_ASSIGNED  [Pack One Amendment One]  live
  Verdict grammar; signature Ichii Kessai from Lion Stance, no defensive form; answers plate with fault-line propagation, ranking it and cutting where already stressed; canonical, transcribed not invented.

R1-1-THREE_VOCABULARIES_RULING  [Pack One Amendment One]  live
  WOTR uses three combat vocabularies (Blade, Verdict, Percussion), not one; the vocabulary is assigned before the technique.

R1-1-VERDICT_GRAMMAR  [Pack One Amendment One]  live
  Applies to characters whose combat is a reading followed by a conclusion, with no conversation because a conclusion doesn't converse; the prose skeleton is read, rank, commit, and the tension lives in the read; the beat before the strike is longer than the strike.

R1-1-YOKO_ASSIGNED  [Pack One Amendment One]  live
  Blade grammar; works from the bind, nose reads before eyes so her Indes is genuinely faster than her sight allows, an unfair advantage she does not comment on; answers plate with half-swording into gaps.

R1-4-PRECEDENCE_NOTE  [Pack One Amendment Four]  live
  The Combat Craft Guide governs duels and small actions; the Mass Combat Craft Guide takes precedence the moment a formation exists, and duel instruments (the three-layer hit model, HEMA vocabulary, armour-tier breakdown) are suspended or replaced accordingly.

## Docket

`check_docket prose-law magic-mechanism combat codex`:

-- 0 pending/proposed for ['codex', 'combat', 'magic-mechanism', 'prose-law']

Nothing pending or proposed for these tags, so no escalation.

## Scene brief

`scene_brief "<beat>" --thread "Wystan Ashmore" --type working --culture Accord --cast "Vesk" "Malphas" "Ilmar Foss" "Nuvalik" "Xu Deming"`, from "Fill before drafting" up to the loadout. The loadout is dropped here because it is in §3. The tool heads the brief "working — Wystan Ashmore" because the thread is his; the POV is **Vesk**. Blank 1 is Vesk (outline). Blank 9 is set piece, 3,500 words. Blanks 4 and 6 have candidates in the Protocol; blank 3 (ignorance quota) has one there too (the hands keeping colour away from the press). Blanks 10, 11 and 13 are "none: no contest" (there is no fight). **Malphas's FOW line below is the lich card: none of it reaches the page (bible).**

### Fill before drafting (author notes)
1. POV: ____  (write from whoever knows least about what is coming, R5-C1-WRITE_FROM_LEAST_KNOWING; a default, breakable with reason)
2. This POV knows: ____ | wrongly believes: ____ | the reader knows that they do not: ____  (R5-C1-INFO_TRACKED_PER_POV)
3. Ignorance quota, one thing the POV notices and cannot interpret, unresolved this scene: ____  (R6-3-IGNORANCE_QUOTA)
4. Misreading, one confident inference that is wrong and stays wrong, well-reasoned: ____  (R6-4-MISREADING_BUDGET, R5-C1-WELL_REASONED_WRONG_CONCLUSION)
5. What it costs, specific and visible (reserve, body, debt, reputation, who saw): ____  (Table Rule 5; R12-1-PACK_SEVEN_SURVIVORS)
6. The lie: one thing an NPC says that is wrong and stays uncorrected (once per session): ____  (Table Rule 8)
7. Standing Inventory, two minimum (Accord): ____ and ____   (R6-9-RECURRENCE_RULE)
8. Ends on: ____  (physical action, an NPC line, or a thing he can now see; never a question at Isaac. Table Rule 1)
9. Length band: ____  (conversational 300–700 / standard 700–1,500 / set piece 2,500+; default the middle. Table Rule 2)
10. Each fighter's combat vocabulary, assigned before the technique (Blade / Verdict / Percussion / Expenditure): ____  (R1-1-THREE_VOCABULARIES_RULING)
11. The read: what evidence the POV gets, over which two or three exchanges, before anything kills (R12-3-COMBAT_EXCHANGE_OWES, R12-3-NAMED_INVENTOR_RULE): ____
12. Which two of the four explaining voices carry the mechanism (R12-4-VOICE_MIXING_RULE): ____
13. Stat that decides each contested action, by row (R14-3-STATS_DECIDE_TABLE, R14-3-TRACEABILITY): ____

### FOW lines (never invent a number)
#### Vesk  (Volume I — Character Cards/Vesk.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

#### Malphas · Ferment of Stars  (Volume I — Character Cards/Malphas · Ferment of Stars.md)
**Crystal State** · Overgrown, held there permanently rather than resolving. **η · 0.78–0.84, Band SSS.** A living Crystal this overloaded fractures or crystallizes within a season. His does neither. The split housing does not let the pressure resolve — it lets the pressure continue, indefinitely, which is a different thing from surviving it.
##### IV · Stats
**Stage XIV — Zenith** · **Level 475** · Level Band V · Coherence Band SSS · Grade EX with the array at ceiling on his flagship stat · Ceiling 1,500.
**Pool 23,100** (14,000 leveling + 9,100 Thresholds through Stage XIII). Allocated 23,095.
| Stat | Value | Grade |
|---|---|---|
| **Ardency** | **1,500** | EX |
| Resilience | 1,411 | EX |
| Tempering | 1,344 | EX |
| Gnosis | 1,272 | EX |
| Vitality | 1,200 | X |

#### Ilmar Foss  (Volume I — Character Cards/Ilmar Foss.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

#### Nuvalik  (Volume I — Character Cards/Nuvalik.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

#### Xu Deming  (Volume I — Character Cards/Xu Deming.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

## Previous chapter

The last 1,421 words of `ch09/final.md` (of 3,395), raw, cut at a paragraph boundary. Ch9 is Wystan's side of the line; nothing in it reaches the coldhouse.

"Twice."

"Yesterday and today. Yesterday it did nothing, and I had it wrong, and the wrong one is in the book as well."

"Good."

She looked up at that, and then away.

"It does nothing in the flask," she said. "It has done nothing in the flask for a day and a half. I have had it warm, I have had it cold, I have had it standing in the light. Nothing. Not a trace of it, not a..." She stopped and started again. "A weak thing does a little. This does none."

"And in the pail."

"Watch the pail."

She took the cover off. Wystan had the smell of it at once, iron and warmth and the shambles, and under it something green at the back of his tongue, which he had had before and had not written down then either.

She put a very little of the flask into the pail with the end of a glass rod, and stirred it twice, and stood back with her hands behind her.

Nothing happened for the length of a slow count. Then the surface went from red to a duller red, from the middle out, the way a skin comes over milk off the boil, and the whole of it drew in a little from the sides of the pail. A ring of wet showed round it on the tin. The colour kept going while he watched it and did not stop going.

"There," she said.

"How long does that run?"

"I don't know yet. Yesterday's did not run at all so I have one number and it is from an hour ago." She picked up her pencil and put the time in the book. "It wants blood. Not damp, not heat, not standing in the light. Blood, and it crosses over in blood, and once it has crossed it goes on in the meat with nothing left to hold it up."

Wystan stood over the pail with his hands in his coat pockets and looked at the ring of wet on the tin.

"The tin at Ferriby has gone on running under a seal," he said.

"I know what the tin has done." She had the pencil still. "I am not putting that in the chapter book off one pail of pig at one hour, and you would not let me if I tried. The pail is a pail."

"No. Write the pail."

"I am writing the pail." She wrote it. "I am not writing yours."

He let that sit. She had her head down over the book and her wrist across the page, and the regulation bracelet had come forward on it out of her cuff, the badly struck mark on the plate turned in toward the skin. She wrote four lines, and did not look at it, and did not shift her wrist.

"Nothing stood up in there," he said. "Nothing leaned."

"No. It isn't that." She kept writing. "That's the other half of it and it is not in the pail. This is only the part that starts."

"Then write only that."

"What do you think I'm doing?"

He went along the bench to the window and stood out of her light. Below in the yard the harness-maker had a hide over a trestle and was working at it with both arms. The green was still at the back of Wystan's tongue. He held it there and put nothing against it, because he had nothing to put, and it was not the room, and he had not touched the pail.

"There was a price today," he said.

"Of what?"

"Of the pour at Ferriby. All of it. At the tariff."

She set the pencil down in the spine of the book. "Say it."

"Two myriads and a fraction."

Yinzhi said nothing for a moment. Then she took her hands off the bench and put them under her apron, which he had seen her do once before, in the scullery, when he had told her what would be said about her at a hearing.

"For deer."

"For eleven hinds and a stag."

"Two myriads." She turned and looked at the meter by the door, and then at the draw-tap at the end of the bench over the stone trough, and back at him. "The chapter draws at third for a week and it is on our bill at the month and the bill comes to the secretary and the secretary complains at me about it. At the month. For a week." She stopped. "Who has a bill like that."

"Somebody does."

"Somebody paid it."

"Yes."

"Then it is on a page somewhere with his name over it." She had come up two steps toward him and stopped. "Wystan. That is the kind of thing your Division is for."

"I asked Logistics for the draw on four lines this afternoon."

"And?"

He took the slip out of the day book and gave it to her, and she read it and gave it back.

"Nine words," she said.

"Nine."

"How long is the schedule?"

"It is a return to the quarter. I don't know the day."

"So you wait until a room in the capital has had its quarter." She went back down the bench to the pail, and put the cover on it, and stood with her hand flat on the cover. "It will have gone cold by then."

"It will have published by then," he said. "That is a different thing."

The chapter's shutters were open on the north light and the light had gone the colour it went at the tenth. Wystan read her four lines in the book, holding the leaf by its corner, and did not follow the middle of them, and read them again. He signed nothing. It was her book and her hand and her reading, and that was the whole use of it.

He went back to Cutler Row at the eleventh.

The day desk was down to one clerk and the lamps were lit. He wrote the request out on the Register's form at the end of the counter, standing, because sitting down at the day desk invited a conversation.

A standing request against the Swale line of the northern arrays. The quarterly Logistics return for the line, and the draw entered against it, to be brought to the Night Register on the day of its publication. Not when it was asked for again. On the day.

He put nothing in it about Ferriby, and nothing about the other three lines, and no figure of any kind. He signed it the way the Register signed, *Night Register, Cutler Row*, and no name, and struck his brass into the wax while it was soft, with his elbow on the counter and his left hand pressed over the back of his right so that the tremor went down into the counter instead of into the seal.

"Standing," the clerk said, reading it upside down.

"Standing."

"Against the one line."

"Against the one line."

The clerk turned to the registry roll and entered it in ink: the date, the division, the office, the line, and *standing* in the last column. He wrote out the counterfoil, tore it off along the rule, and slid it across.

Under the floor the main went off.

It went the way it always went, down through a half-note and then out, and the boards under Wystan's boots stopped carrying anything, and the lamp by the clerk's hand was suddenly the loudest thing in the room. The clerk said what he said about it every night at the eleventh, and Wystan did not answer him.

He took the counterfoil down the passage to the scullery in the dark he knew by the walls. The tin was on the sill under his brass where he had left it, and the block's tin beside it, and the fold of chapter wax squared to the edge.

He set the third lamp going and opened the day book at the leaf with the two myriads on it. The wire's slip was already at that leaf. He laid the counterfoil in beside it, squared the two of them to the gutter, and wrote in the margin, in the division's figures, the day the request had been entered and the line it stood against.

Then he sat with his hands flat on the leaf and read the last column of it again, where the clerk's hand had put the one word, and the word said *standing*.

## Cast

Every cast member has facts in state.json, so each gets the `fow_line`, not the full card (the facts are under Continuity). The bible line follows each.

### Vesk (POV)

state.json has 14 facts about Vesk (subject "Vesk").

#### Vesk  (Volume I — Character Cards/Vesk.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

bible.md: "**Vesk.** Card stub. Formulator; grafted hands that no longer close, ridged yellow nails; the village that put stones on its graves; counts the drip. Dies ch45 of his own Verdantia in his grafts; walks; stones."

### Malphas

state.json has 14 facts about Malphas (subject "Malphas").

#### Malphas · Ferment of Stars  (Volume I — Character Cards/Malphas · Ferment of Stars.md)
**Crystal State** · Overgrown, held there permanently rather than resolving. **η · 0.78–0.84, Band SSS.** A living Crystal this overloaded fractures or crystallizes within a season. His does neither. The split housing does not let the pressure resolve — it lets the pressure continue, indefinitely, which is a different thing from surviving it.
##### IV · Stats
**Stage XIV — Zenith** · **Level 475** · Level Band V · Coherence Band SSS · Grade EX with the array at ceiling on his flagship stat · Ceiling 1,500.
**Pool 23,100** (14,000 leveling + 9,100 Thresholds through Stage XIII). Allocated 23,095.
| Stat | Value | Grade |
|---|---|---|
| **Ardency** | **1,500** | EX |
| Resilience | 1,411 | EX |
| Tempering | 1,344 | EX |
| Gnosis | 1,272 | EX |
| Vitality | 1,200 | X |
| Harmonics | 1,116 | X |
| Dominion | 1,032 | X |
| Dexterity | 924 | SSS |
**Path** · Spirit dominant, Fate secondary.
**Peaks** · **Ardency Saturation 1,500** *(at ceiling)* · Resilience Severance 1,464 · Tempering Conversion 1,392 · Ardency Spectral 1,356 · Resilience Immunity 1,320 · Vitality Filtration 1,284 · Gnosis Adaptation 1,248 · Ardency Detonation 1,212 · Tempering Processing 1,176 · Resilience Nullity 1,140 · Gnosis Diagnosis 1,128 · Harmonics Voidance 1,104.
> **Ardency Conversion · 400 · frozen at A-Grade.** Body Path at Stage V required, and the requirement is no longer merely unmet. It is unreachable. He gave up the living body the path was written for.
>
> **The Living Cauldron still cannot convert at volume, and now he never will.** Saturation sits at ceiling. Conversion sits exactly where it stood at Stage IX, five full Grades behind everything else he owns. Every level since has widened the gap rather than closed it.
> *Feed him faster than he can convert and he does not grow stronger. He backs up. He has been backing up since before he died, and the split housing is the only reason the backup hasn't already ended him.*
---

bible.md: "**Malphas (the living man, pre-Becoming).** The card is the lich form (Stage XIV Zenith, Level 475, Class VI Voidic, Dissolution/Mortalis/Solfatara/Nihiloth, the Golden Remnant). **None of it reaches the page.** The prose man is living, not large, fills no room, sits with his forearms on his knees, reads a forester's life like a bill of lading, and his hands never take the cold. No figure exists for him alive; no Stage is named or implied. Expelled from the Pyraeon Forge Academy (card); the book never says so on the page."

### Ilmar Foss

state.json has 19 facts about Foss (subject "Ilmar Foss", and 'Foss').

#### Ilmar Foss  (Volume I — Character Cards/Ilmar Foss.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

bible.md: "**Ilmar Foss.** Card stub. Instrument-maker struck from the Hexagonal Oath for "a rounding convention"; steadiest hands; explains like a man expecting an appeal. Released ch68; lives under his own name, informs no one (ch70). Roll-name question (R23-11) left open."

### Nuvalik

state.json has 8 facts about Nuvalik (subject "Nuvalik").

#### Nuvalik  (Volume I — Character Cards/Nuvalik.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

bible.md: "**Nuvalik.** Card stub. Far-Northern; carries a name three people carried; wipes the blade before she speaks; the sky proverb. Leaves ch55 with Tally's rule, informing no one."

### Xu Deming

state.json has 10 facts about Deming (subject "Deming").

#### Xu Deming  (Volume I — Character Cards/Xu Deming.md)
**Level / Stage / Band.** Pending Isaac.
###### IV–V · Stats
Pending Isaac.
###### VI–VII · Force and Flow
###### VIII–IX · Traits and Domain
###### XVI · Temperance Record

bible.md: "**Xu Deming.** Card stub. The purse. Hat in his hands, thumb round the brim; careful diction; thirty-one days, eleven weeks. Convicted as payer ch60, refuses to name the hand."

### Voice fingerprints

`voice_fingerprints`: of this cast only **Malphas** has the eight attributed lines the table needs:

| character | lines | mean words | short share | questions | contractions | first person | imperatives | favourite words |
|---|---|---|---|---|---|---|---|---|
| Malphas | 8 | 32 ±19.0 | 0.0 | 0.0 | 0.25 | 0.38 | 0.12 | objection, both, says, stays, where, written |

Long sentences, no short lines, no questions: he states. His ch10 line is the row's, "the sink is the quiet", and he stays in that register. Vesk, Nuvalik, Foss and Deming have too few lines to measure. Keep them unswappable by the source and the chapters (R15-1-VOICE_DIFFERENTIATION): Vesk says things too loud and is grateful to be corrected (ch2); Nuvalik wipes the blade before she speaks and is flat and anatomical ("She is meat with a new net in it and the net can shorten."); Foss explains like a man expecting an appeal ("It is filthy, and it is correct, and those are two separate findings and I want both in the book."); Deming has careful diction and says things for the record ("In eleven weeks somebody in a room in the capital will look at the Swale line and see a bad spring.").

## Recall

### scene_recall "ninth hind stands table leans standpipe housing gauge bench coldhouse sink quiet matrix walks"

Only the first hit (the source) is on this thread; the rest are off-thread and are kept as returned.

#### The Night's Watch  (the_nights_watch.md, score 81)
[Author notes] Isaac's scene, archived as delivered. Three movements: The Thing With Edges (Wystan, Night Register, Cutler Row), What Does Not Smell Dead (Vesk POV, Malphas's cell at the Greyshaft Nine coldhouse; Project Zombification named), Forty-Two (Yinzhi's sub-floor sweep, the hind signature, Wystan's anamnetic contact). New na...
[WHAT DOES NOT SMELL DEAD] The main under Greyshaft Nine had been capped four years and the building still held its cold the way a cellar holds it, in the slate and the brass and the iron of the standpipe housings nobody had bothered to strip. Water came off the ceiling in the far corner at a rate Vesk had counted once and never stopped hearing....
...ed forty miles of timber every week and counted deer for a living. His insert said that on the morning of the ninth he had come down the Ferriby cut and found eleven hinds and a stag inside a quarter mile, all of them dead wh...
...Her jaw moved. "The clerk said it was."  "And you are here because."  "Because I sealed his sample tin on the ninth, in the cut, standing in it, and I assayed the residue at the chapter bench on the tenth with a coil I had ca...

#### Commencement of The Curia. [ Kujo Arc ]  (commencement_of_the_curia_kujo_arc.md, score 59)
...ollowing it.  The priests at Altherion had a name for this. They put it in the mouths of the dying and in the ninth-year sermons and on four walls that Verinus had walked past every winter of his working life. The Song of Gra...
...And the fourteen were in it. That was the thing the walls at Altherion had left out, and the sermons, and the ninth-year readings, and Verinus heard it and understood at seventy-one what he had been getting written out of min...

#### The_Path_of_Sorrow  (The_Path_of_Sorrow.md, score 46)
[The_Path_of_Sorrow] The brush moved again. Heavier this time. Closer. The Kamigan read the displacement pattern — the way the branches moved, the timing of the movement, the weight distribution implied by the angle of deflection — and constructed a silhouette: quadrupedal, approximately 400 kilograms, low-slung through the shoulders, wide...
[The_Path_of_Sorrow] "I can't protect her here. I lost my brother. I lost my mother. I lost everything." Each sentence arrived as a separate unit, placed beside the previous one the way stones are placed beside other stones in a wall, and the wall was not being built for shelter, it was being built for containment, each stone sealing in th...
...d and far away, as though the world had been wrapped in wet cloth and was now breathing through it. The inn behind Temür was a two-story timber-and-daub structure with a sagging roof and a chimney that leaked smoke from a cr...
...none of that mattered because the room had a door and the door had a lock and the lock worked and Emira was behind it, which made it the most valuable room in the township.  Temür sat on the inn's front step with his back ag...

#### WAR OF THE REALMS  (WOTR_Vaeloris_Sequence.md, score 45)
[The Cabin, III: The Lowest Ardency Ever Assayed] "So if there is one thing I would like to arrange before the ninth, it is not my survival. It is a kingdom that does not require it." He looked across the table then, and something in his face eased for the first time since he had come through the door.
[The Cabin, II: Two Halves of the Same Wall] ---
...le** · The Rimward wall. Verinus, Barrow, Fend. 11. **The Slope** · Verinus and Haruki, two days out from the ninth.  ---  ## PRINCIPAL FIGURES  **Verinus VII**, born Osric Peyser. Palatine of the Holy Sea of Alabaster, Keepe...
...The channel is deeper." Verinus's voice remained exactly where it had been. "I sounded the Ebb Channel on the ninth of the month, as I have sounded it on the ninth of every month for nineteen years, and it read four fathoms a...

#### The War in the North I — The Kharven-Seat  (the_war_in_the_north_i_the_kharven_seat.md, score 43)
[Author notes] Standing Inventory, recurrence rule R6-9. The scene runs five signature Kharven items rather than the two-minimum: the woodpile and the count, the night-stone, "wet wood" as the insult behind the eleven-short stack, the Thin Weeks in Lambert's arithmetic, and the third bowl carried by the Waiting's absence. Nothing inv...
[The War in the North I — The Kharven-Seat] *Six days I walked and the first thing out of my mouth is the woodpile. My mother would have laughed until she was sick.* Rannvei put her back to the wall next to the door. *Say the rest of it plain. They are all going to want it dressed up and it does not dress.* "Cold Cross to the Whitmoor cut is a day and a half," s...
...hreaded collar at one end and a hairline glass window let into the face of it. The glass had a scale etched behind it in fine black strokes and a needle sitting dead at the low stop. Along the shoulder, struck deep and struc...
...hat I judged, in case I judged it wrong."  Yoko came off the wall.  She crossed to the table, put her hands behind her back so that they would not be doing anything, and lowered her face to within a foot of the brass without...

#### The Vacancy  (01_the_vacancy_korvaeth_arc.md, score 42)
[VI. THE FIRST YEAR] It had gone quiet without going hostile and without declaring. Every barrow-ground east of the Vessel line had acquired, over the nine days after the yard, the specific stillness of a place already grieved for, and it held that stillness with nobody maintaining it, and no Tsohanto delegation came to any hall of either ...
[V. THE COURTS ANSWER] Every Concord observer present transcribed the exchange as having had nothing said in it. A four-count is assent. A nine-count is a refusal that will not be argued. A silence held past a nine-count has moved out of language and into judgement, and the Winter Court had just passed sentence on a sovereign claimant in fro...
...ame up the Vessel road in a Reckoner's case with a Reckoner's seal on it and reached the reading-floor on the ninth day, and Aeren Lyth read it first and Sylvenn read it second, and they got four lines in before Aeren stopped...
...ould not: the entry was clean, the Master of the Floor had said so on the fourth day and it had leaked by the ninth. What the Court did instead was convene, in full form, with the count called and the witnesses compensated, a...

#### The Fire of the Undeserving  (01_verinus_fire_of_the_undeserving.md, score 41)
[The Fire of the Undeserving] *Deepvein station, the Nalūn basin. Extract from the Fixatio watch-log. Same night, fourth hour.* The salt rang. The station floor moved under the bench. Every sealed instrument in the lower vaults took weight at once and the chain-bridges at Ironlink went to their stops and hung there. No drawdown at any basin site. S...
[Notes (author-facing, not canon)] **Originated, pending ruling.** The Rubric of Sacrament and its five recorded failures. The Deepvein log. The Cloister Ward and a metered Well under the Grand Church. The Altherion understructure answering weight (the Scale beyond the Alabaster cathedral). The gold staff as the Palatine's object of office. Judger's Fla...
...The Cloister Ward drawing on the Well under the Church through the night, through sealed conduit somewhere behind the wall at his back, a long low note with a grain in it and once in a great while, far overhead, the click o...
...der the eleven strips on the floor and did not disturb them. It ran under Sesk's crate. It reached the wall behind the lamp and climbed it a hand's width and stopped, and the beeswax in the frame at Aurelian's back began, ve...

#### Nine Hours of Daylight  (nine_hours_of_daylight.md, score 37)
[Nine Hours of Daylight] *Coldwater common, continued.* *POV: Ruvaen Aloreth. RP reply. Dova'Kan, Gorgi and Xmere are not written; their beats stay open.* ---
[V. THE SEAM] "None. I am inventing one and Corvienne Sorreth will hate it and she will draft it properly by tomorrow evening because she hates it, and until she does, the instrument is that I said so." He turned his head. "And Sergeant. Whoever guards it does not know what it is. Tell them it is chalk." *Which is the whole trouble ...

### scene_recall "Vesk Malphas Ilmar Foss Nuvalik Xu Deming Greyshaft Nine coldhouse"

Trimmed under the trim order (the brief sits near 40k tokens; the wiki snippets went first, then this). Its only on-thread hit was *The Night's Watch* (the source, score 99): the "What Does Not Smell Dead" movement, Vesk's POV at the coldhouse, with the lines "the iron of the standpipe housings nobody had bothered to strip" and "Nine hinds hung on the rail. They had been on the rail eleven days." The first recall already carries the source. The rest were off-thread (Nine Hours of Daylight, The War in the North I, In Form, Recalescence, A Silence in the Chord, The Hatch, No Entry For That).

### wiki

Trimmed under the trim order (wiki snippets go first). The beat's only capitalised term that is not a cast name is "Greyshaft Nine". `wiki "Greyshaft Nine"` returned only the cast's own stub cards (Ilmar Foss, Nuvalik, Vesk: "Malphas's cell, Greyshaft Nine coldhouse. ... Scene-derived stub from The Night's Watch"). It has no page for the place. Everything known about the coldhouse is in the chapters (see Continuity and the Protocol).

## Continuity

### Facts (cast or place, newest first, at most 40; lies marked **[LIE]**)

<!-- 71 matched; showing 40 -->
- ch8 · Foss · object: Foss keeps a little book for readings and writes in it which Swale walk has a white salt lick with hinds at it. — "Then he took out the little book he kept for readings and wrote down which walk it was."
- ch8 · Foss · promise: Foss is to be released with his own name when the work is done; he does not know which name that will be. — "When the work was done he would be released with his own name."
- ch8 · Foss · fact: Foss has given his rounding account to the examiners, a clerk of the Oath, a woman he once thought he might marry, and Vesk, drunk; never yet to anyone asking. — "He had given it to Vesk, drunk."
- ch8 · Foss · fact: Foss's rounding-convention account (his rehearsal, not certified): the Oath enters at the second place with fives to even; his issued instrument carried fives up; 'every figure I entered was correct to the instrument' for eleven years. — "For eleven years every figure I entered was correct to the instrument."
- ch8 · Foss · fact: Foss counts the Greyshaft drip at twenty-two. — "The drip. Foss counted it without meaning to, and made it twenty-two, and stopped."
- ch8 · Malphas · fact: Malphas's hands keep their colour at four degrees while Foss's go white at the nails; Foss has no instrument for it and stops looking. — "They were the colour of hands."
- ch8 · Malphas · fact: Malphas orders Foss next turn to take the carter the walks. — "Next turn you take the carter the walks."
- ch8 · Malphas · fact: Malphas declines Foss's self-blame about the road; it is not written in the book. — "What you think of it is not a reading."
- ch8 · Malphas · fact: Malphas asserts the lick-keeper 'is exactly what he says he is'. This is his belief only and is wrong: Josse lied about the barn (ch7). Do not treat as true. — "They will have the lane, and at the end of the lane a man who sets salt for his living and was paid to set it, and he is exactly what he says he is."
- ch8 · Malphas · fact: Malphas moves the licks to the Swale walks, one block to a keeper to begin with, where keepers already set salt. — ""The licks go to the Swale walks.""
- ch8 · Greyshaft Nine · fact: Foss cut the three house rules on the door board on Day 5; the knife skipped once in the S of 'season'. — "The knife had skipped once in the S of season."
- ch8 · Greyshaft Nine · time: Foss reaches Greyshaft Nine at the first division of Day 9, having ridden through the night; its main has been capped four years. — "The main had been capped four years, and Foss had long ago stopped expecting anything from the floor when he came in."
- ch8 · Foss · misreading: MISREADING (uncorrected): Foss concludes the Register reached Ferriby up the road, carter by carter, because of his own asking after the manual. Actually it came via Tally's survey stone, Called Late and the estate's round. — "The Register had come to Ferriby the way he had come to the book, carter by carter, asking, until it found a man who carried something up a lane."
- ch8 · Foss · fact: Foss spent a season asking on the Swale road (carters, chapmen, a horn-spoon woman at the crossing) for the Division's field manual. — "He had asked carters and chapmen and a woman who sold horn spoons at the crossing below the inn, a little at a time and never twice to the same face, whether anybody had a grey limp-covered thing with the Division's heads in it, because a man he did work for wanted to read a paragraph."
- ch8 · Foss · fact: Foss refuses to wire: a wire is nine words and the flimsy is kept at both ends for a turn. A wire office stands at the crossing below the inn. — "There's a wire office at the crossing."
- ch6 · Nuvalik · fact: Nuvalik sees nothing to be released to: her name was carried by three before her and spent in this room. — "There was nothing to release her to."
- ch6 · Nuvalik · misreading: Nuvalik believes Malphas's hands keep colour because he comes from a warm building and sits only a division; she has no control case. — "He came from a warm building, and he sat a division, and he went back to a warm building, and a division was not long enough to take the colour out of a man who was not working in it."
- ch6 · Malphas · fact: Nuvalik sees Malphas's hands keep a living colour at four degrees while hers go the colour of tallow. — "The gauge over the door had stood at four degrees since the first division and her own fingers had gone the colour of tallow inside the first quarter of it, the way they did every day, and his were the colour of hands."
- ch6 · Vesk · number: Vesk has had all nine hinds through his hands and nothing like the contraction from any other. — "I have had nothing like it off any of the others and I have had all nine of them through my hands."
- ch6 · Vesk · injury: Vesk's two ridged yellow graft nails have grown out again since the fourth day. — "The two nails on the fingers that had taken stood up over the edge of the bench, ridged and yellow and grown out again since the fourth day, and he was looking at them the way she looked at a joint she did not like."
- ch6 · Vesk · object: Vesk wears the yard gloves, canvas, the only pair in the building; his hands do not close fully in them. — "They were the yard gloves, canvas, stiff with the cold, the only pair in the building, and he stood a long moment with his hands in them and his hands not closing all the way."
- ch6 · Vesk · fact: The sealed make-up took longer than it ever had, and nobody remarked on it. — "Then he made the dilution up inside the flask with the seal on, and it took longer than it had ever taken, and nobody said anything about that either."
- ch6 · Vesk · lie **[LIE]**: Vesk claims the dilution can be tempered sealed, half a division slower. He has never done it and does not know. — ""It can be tempered sealed," he said."
- ch6 · Foss · number: Foss held a numbered instrument for eleven years and was struck off for a rounding convention. — "I held a numbered instrument for eleven years."
- ch6 · Foss · name: Foss's given name is Ilmar. — "Ilmar Foss, on a bench, with hands."
- ch6 · Foss · promise: Malphas promises Foss release under his own name when the work is done, and not to ask or send anyone after; it goes in the book. — "When the work is done you will be released with your own name."
- ch6 · Malphas · fact: Malphas put his hand on the poured ninth hind's flank on the first night; he calls it a bad habit that stops now. — "You put your hand on the ninth hind's flank on the first night."
- ch6 · Malphas · fact: Malphas spent two years designing against the paragraph from hearsay; three counters right, one backwards: he believed attribution ran up from object to hand, so the crew touched flasks bare. He puts the error in the book. — "I have had you all touching flasks bare because I believed it ran the other way, up, from the object to the hand."
- ch6 · Malphas · fact: Malphas had the fourth counter (move a wall) read twice and did nothing visible with it. — "Read the last one again," Malphas said."
- ch6 · Foss · fact: Foss obtained the paragraph a season earlier than he had said. — "I said a season and I was wrong by a season."
- ch6 · Nuvalik · fact: Nuvalik has said aloud that the reproduction will not be as clean as the fourth hind. — "It will not be as clean," she said."
- ch6 · Foss · fact: Foss is to hold the eighth hind's joint while Nuvalik goes in, so he cannot be told he expected the draw. — "He has the steadier hands and he has never had them in one, and if it comes in under his hand as well he cannot be told he expected it."
- ch6 · Malphas · fact: Malphas now knows of all three contractions and that Nuvalik held them back; Vesk and Foss heard it too. — "Why is it not in the book," Malphas said."
- ch6 · Nuvalik · fact: Nuvalik kept the contraction out of the book deliberately, so that no one would pour the hind again and lose what it was doing. — "I did not write it down so that nobody could spend it."
- ch4 · Deming · fact: Deming tells the Hollow Shaft story aloud to the factor, as a joke about careless men. — "That is how they found the Hollow Shaft people."
- ch4 · Deming · misreading: Deming concludes the factor is a carriage man who lost interest in the Swale page once carriage was lost to him. Not true: do not treat as fact. — "Once the carriage was lost to him, the Swale page was the capital's trouble and no longer his."
- ch4 · Deming · promise: Deming tells the factor that in eleven weeks the capital will see a bad spring on the Swale line, and asks for it noted. — "In eleven weeks somebody in a room in the capital will look at the Swale line and see a bad spring."
- ch4 · Deming · fact: Deming tells the factor the carters' notes are clean and every cart up the Swale road this quarter went full (true). — "Every cart that went up the Swale road this quarter went up full and was entered full."
- ch4 · Deming · number: From the house's own copies of old returns, Deming derives eleven weeks from the factor taking the page to the published Logistics return coming back down the river. — "It came out at eleven weeks."
- ch4 · Deming · fact: Deming knows Malphas counted the thirty-one days before he did and let him argue anyway. — "Malphas had counted them already."

The 31 older matches not listed are ch4 and ch2 facts. The ones the POV needs are carried in the Protocol and the Cast: Vesk's hands close two thirds of the way and he carries in flat palms; his two ridged yellow graft nails; his village's beck stones on graves; his ch2 misreading ("The press threw its warm air along the east wall and the gauge bench stood in the middle of it, and that was the whole of why the Lord's hands never took the cold."); the drip at twenty-two; the housing gauge at four degrees; the standpipe housings dead along the west wall; Nuvalik's ch2 **[LIE]** ("Everything the fourth hind gave me is written down.", paid ch6); Deming's ch4 **[LIE]** ("Cause: the thaw.").

### Related facts: the ninth hind, the eighth hind, the house rules and the book (not cast or place; for the stand)

- ch8 · the project book · object: The order is written in the project book in Foss's hand, with the carter's words, and initialled I. F. — "He wrote *I. F.* at the foot, and then, because it was his hand and it was in the book, he could not leave it there."
- ch6 · the ninth hind · fact: At chapter end Nuvalik takes the big knife to open the ninth hind. — "She went back in and took the big knife off the hook to start the ninth hind's hide off her, because a hind that had done nothing in five days was a hind you learned from by opening."
- ch6 · the ninth hind · fact: The ninth hind, under sacking since the first night, has done nothing; nothing written on her in three days. — "Nuvalik had written nothing about her in three days because there had been nothing to write."
- ch6 · house rules · object: Foss cuts the three rules into a board for the door, since ink on a coldhouse door would not last a turn. — "Behind her Foss was cutting the three rules into a board with a knife."
- ch6 · house rules · fact: The season and the wall are not house rules but the end of the work; written under the three with a rule across. — "Write them both under the three and put a rule across, so the two do not read as instructions."
- ch6 · house rules · fact: House rule three: nothing from a flask or the substrate reaches Malphas without a second pair of hands; Vesk makes up, Nuvalik or Foss carries, Nuvalik cuts; Malphas touches neither dose nor substrate. — "Third: nothing comes from a flask to me, or from the substrate to me, without a second pair of hands between."
- ch6 · house rules · fact: House rule two: no live dose touched bare; gloves on before the seal comes off; gloves stay in the room. — "Second: no live dose is touched bare."
- ch6 · house rules · fact: House rule one: every flask Fixatio-sealed when made up, seal broken only at the table, resealed over what is left; nothing tempers open. — "First: every flask is Fixatio-sealed when it is made up, and the seal is not broken until the flask is at the table, and it goes on again over what is left."
- ch6 · the eighth hind · time: Because the sealed make-up tempers slower, the eighth hind is poured at the fifth division and stands a division less than the fourth before Nuvalik goes in; the book records it as a division less, not the same (figure rests on Vesk's lie). — "If it tempers half a division slower, she is poured at the fifth division and not the fourth, and she stands a division less than the fourth hind did before I go in at her."
- ch6 · the eighth hind · object: The eighth hind was poured sealed at four places at the fourth hind's dilution, count of forty each, two hands, and written up in the book. — "The small knocks went on while she wrote the eighth hind up in the ordinary way: four places, the count of forty, poured sealed, two hands."
- ch6 · the eighth hind · time: The eighth hind came off the rail hook at the fourth division of Day 5. — "The eighth hind came down off the hook at the fourth division."
- ch6 · the eighth hind · fact: Malphas orders the contraction reproduced: a drawing shoulder by the eighth division tomorrow (Day 6), under two different pairs of hands, recorded in the book by both. — "By the eighth division tomorrow I want a shoulder that draws, and I want two hands on it that are not the same two hands, and I want it in the book in both."
- ch6 · the book · object: The contraction is entered in the project's book in Nuvalik's hand with the concealment dated: 'held back by me, three days, so that nobody could spend it'. It is the first entry that has her in it. — "It was the first entry in the book in her hand that had her in it at all."
- ch6 · the fourth hind · fact: The fourth hind is finished now; Nuvalik found nothing in her on the morning of Day 5. — "She is finished now."
- ch6 · the fourth hind · fact: The fourth hind's shoulder drew three times under Nuvalik's point: twice on her second day, once on her third, each smaller. — "Twice on the second, once on the third."
- ch2 · the ninth hind · fact: The hind does not stand. The hide at her shoulder draws up once, a hand's breadth, untouched; Vesk tells nobody and does not call Foss's glass to it. — "He did not call Foss back to put the glass on it."
- ch2 · the ninth hind · fact: No anchor in the hind and there will not be one (an anchor is a binding, a binding takes a name); she cannot be told where to go, only given somewhere emptier. — "Four to six without an anchor, and there is no anchor in her, and there will not be one."
- ch2 · the ninth hind · fact: Nuvalik states the hind: no Crystal, no self, a net that shortens toward the emptier end. — "She is meat with a new net in it and the net can shorten."
- ch2 · the book · object: The cell's book is kept on the shelf over the ice press, kept by Nuvalik since the first night; nobody else has taken it down. — "It is on the shelf over the press where it has been since the first night and nobody has taken it down but me."
- ch2 · the ninth hind · number: Nine carcasses hang on the rail, eleven days, no smell; the crown growth stands nine inches at best. — "Eleven days on the rail and no smell and nine carcasses signed by nobody, and it had been beautiful work and it had been an argument about what a thing is."
- ch2 · the ninth hind · fact: The ninth hind, eleven days under the ferment, is structurally liquid and cellularly whole; Nuvalik moves her off the rail onto the slate table at the second division. — "Eleven days of the ferment had taken the connective net out of her and left the meat where it was."

### Summaries of prior chapters

**Ch1, The Docket Head.** Wystan checks and blots Yinzhi's full name at the docket head and enters her as assay of record. Gloved, he moves the Ferriby wedge with forceps into a fresh Division Fixatio tin, seals it with brass wax and strikes his brass into the seal. He logs it in the day book as Register file Ferriby, exhibit one, "crown +¼ in sealed." He collects the four pieces of chapter wax. He folds them in a druggist's paper, dated, for custody. Over grounding tea he asks whether the bracelet is still pulling, and Yinzhi says no. He rejects a nine-word wire and writes Tally a letter: walk the line and measure into a notebook, a second dataset from the same observer, who set the marker, and what the animals put their mouths to. He signs it "Night Register, Cutler Row." He enters the Register's position, "A dose goes where it is put," and records the forty-second minute. The main returns at the sixth division. At the day desk he writes his memorandum under Brack's closed classification, with *edges* in the margin. A day clerk reads only the classification and shelves the book.

**Ch2, Twenty-Two Drops.** Day one, the second division to the ninth, at Greyshaft Nine. Nuvalik brings the ninth hind down off the rail onto the slate table. Vesk has built the contractile matrix into the third-dilution formulation on one pour and lets it stand, then carries the flask in both palms and works it in at four places along the near flank, the count of forty between each. The colour clears inland, faster at the two forward pours; Foss, at the glass, gives the reason (the trunk quit last, the forward channel is finest, the finest channel pulls hardest) and calls the synthesis filthy and correct, wanting both findings in the book. Malphas asks Foss to get the Arbitration Division's field manual for its unsigned attribution paragraph; Foss refuses a wire and says it will take a season, and Malphas accepts a season. Nuvalik states what the hind is: no Crystal, no self, a net that shortens toward the emptier end. Malphas orders the night written up in ink under the project's name by four hands. The hind does not stand. Vesk sees the hide at her shoulder draw up once, tells nobody, and counts the drip to twenty-two.

**Ch3, The Morning Check.** Day 1, noon to the ninth division. Wystan wakes with his hands shaking. He recalls Keld's minute, then reads the written record against it word for word. It has not moved. At the medical house the porter admits him by his face, because his brass is on the tin lid at Cutler Row. Keld sits at the window counting under his breath at something Wystan cannot find. He asks "Is that them" and says "They said the sixth." At the day desk Prime Warden Elspeth Lyle reads the edges memorandum and calls the file a curiosity. The Register may keep it open until the quarterly inspection of the gauge-housing and not a day longer. In the upper passage Brack stands by bleed and hands over the Bureau's raw Ferriby field sheets unasked. In Lyle's shut room the position and the term go into her minute in ink, under her silver, and he initials both. Residue settles on his skin and he does not remark it. Lyle orders the sheets entered as received unasked and denies sending for Brack. At the ninth division Wystan tells Brack: "I will get a hand and not a purse."

**Ch4, A Number in the Room.** Day 2, fourth to sixth division, Xu counting house. Deming enters the second tranche of the Swale supply as wastage at the array, where four years of the column had run flat. He writes the covering letter in the house form. Its second line reads "On the account of Project Zombification, second tranche", as Malphas ordered against his argument. The cause given is "the thaw", signed for the house under the Xu chop. On the almanac he privately counts thirty-one days of usefulness and knows Malphas counted them first. He derives eleven weeks to the Logistics return from the house's own copies. The Logistics factor, unnamed, queries the step in the column and pushes to move the loss into carriage on the house's bond. Deming holds it to wastage. He tells the factor that in eleven weeks the capital will see "a bad spring", and has it noted in the factor's pocket-book with the division. The factor reads the letter's second line twice and makes an unexplained hooked mark beside it. He files it between boards, says covering letters go in the stove, and leaves upriver with it after Deming tells the Hollow Shaft story as a joke.

**Ch5, Who Set the Marker.** Day 4, sixth division, to Day 5, sixth. Yinzhi brings the Society's bundle to Cutler Row. Tally's letter: he walks daily again; the marker is the Ferriby estate's old survey stone, their corner, set before the parish was on the roll. It leaves the second question unanswered. Wystan wires the Timberline bench and rides out with Yinzhi on hacks charged to Lyle's bill. At the eleventh division Tally, first seen, shows his notebook without letting go of it and walks them the line. The stone bears a weathered *TERM* and the estate's name. The estate's round comes up through the birch, meets the stone and bends west. Yinzhi's coil confirms the far side clean. Tally says the burn runs both sides and lies that nothing is set out. Wystan stands eight divisions without food and calls the scene late. At the sixth he states: somebody came up from the south along the estate's round, stopped short of the stone, stood a while, and went back the way he came, on foot, at a walking pace, more than once. What he did standing cannot be read. Wystan faints briefly, eats, writes *the round* in the margin, and looks down it.

**Ch6, Separately.** Day 5, third to eighth division, Greyshaft Nine ice-press room. Malphas takes up Nuvalik's ch2 line that everything the fourth hind gave her is written down, against the book's three entries. She admits the shoulder drew three times under her point — twice on the hind's second day, once on her third, each smaller, now finished — and says she kept it out of the book because a thing that twitches is a thing that will walk and a pour would have lost it. Malphas orders it entered in her hand with the concealment dated, and orders it reproduced by the eighth division tomorrow under two hands: the eighth hind down, poured at the fourth's dilution, Foss holding the joint. Foss produces a nameless print of the Division field manual a season early and reads the unsigned paragraph: seal the object, interpose a hand, wait a season, move a wall. Malphas makes the first three house rules (Fixatio seal, no live dose bare, everything through a second pair of hands), leaves the season and the wall as the work's end, and promises Foss release under his own name. The eighth hind is poured sealed. The ninth has done nothing.

**Ch7, The Lick-Keeper.** Day 7. Wystan and Yinzhi walk the Ferriby estate's round north from the home farm. There is a salt lick every half mile, on knee-high oak posts, and the blocks are new tonic blocks, grey with a brown streak. At one post they meet the lick-keeper Josse Garretsohn, first seen, setting blocks bare-handed. Wystan shows the brass and gives only "the Register". Josse says the round turns at the birch, as his father's did, and that the steward paid for the tonic on the split tally. He lies that the blocks come from the home-farm barn and refuses to hand over his half of the tally. He set the birch lick before the thaw. The last lick is inside the birch on the east side, well short of the stone. It sits at Called Late's stopping place: the salt stopped there and the carrier turned home. Hair is rubbed into the bark on the north faces of the trunks round it. Yinzhi's coil shows the block carrying. Wystan grips it bare-handed and takes the forty-third minute: Josse setting it, content, nobody else in it. He concludes the formulator never goes near them. The block is tinned under his brass.

**Ch8, Rounding.** Day 8, the Swale road inn (on the main). Foss pays the unnamed carter five salt bars for two loads to the ford. The bars leave no bill, and the carter shaves one into his pulse. The carter lies that the man at the ford wants two more, on account of the Register. The news: "Man from the Register's been at the Ferriby lick-keeper. Asking about blocks." Nobody in the chain saw him; no name, no description. The carter refuses to give up the man at the ford. The main goes off at the tenth. Foss pays the two and refuses to wire. He concludes that the Register came up the road the way his manual did and blames his own asking. Day 9, the first division, Greyshaft Nine. Malphas asks nothing. He stops Ferriby for a season: no word up the lane, and the posts are left as they are. He moves the licks to the Swale walks, one block to a keeper, written in Foss's hand as the carter said it. He declines Foss's self-blame. Riding home, Foss rehearses his rounding-convention explanation and notes a white lick on a Swale walk.

**Ch9, Two Myriads.** Day 10. In the Division archive Wystan draws the northern schedule of metered draw; the keeper says it is current, and it is a season old. Working from the assay's third dilution and Tally's count, eleven hinds and a stag, with spring weights off the Society's almanac, he prices the Ferriby pour twice and gets two myriads and a fraction both times. He concludes the dose was bought over a meter and that a bill was paid. Off the archive's map he takes four array lines within a day's ride of the Ferriby cut, Swale, Aysgill, Wendle and Scarth, and wires Logistics for their draw records in nine words under the Register's form. The refusal comes back in nine: returns publish on the schedule and not before. At the Timberline bench Yinzhi, working from her own coil reading and touching neither tin, shows him a compound that is idle in the flask and runs in a pail of pig. She will not carry it onto the Ferriby tin. At the eleventh division, as the main goes off, Wystan files a standing request against the Swale line: the return to reach the Night Register on the day it publishes.

### Hook agenda (due within 3 chapters, and overdue)

- **H21-the-ninth-hind** (planted ch2, advanced ch6, due **ch10**). Keywords: `ninth hind`, `table`, `stood`, `standpipe`. **Pays here** (see The beat). Status in state.json: advanced, last event ch6 ("The hide over the scapula lay the way hide lies.").
- Nothing else is due by ch13. The next due are H23-edges-in-the-margin (ch16, Wystan's side) and H16-thirty-one-days (ch22).
- **Overdue:** none. H08 (due 6) was paid in ch6, and H01 (due 7) was paid in ch7.
- Dormant and touchable here, cell side (not due): H11 (last event ch2), H09 (ch6), H10 (ch6), H07 (ch8), H18 (ch4), H16 (ch4). See The beat.

### Audit carry-forward

From `AUDIT_ch05.md` (the latest audit), the critical items and the ones that bind this chapter:

- **Identity: Wystan learns Malphas, or Malphas learns Wystan (CRITICAL if found).** No line has crossed through ch9. The near misses stand: Tally holds "Ashmore" (ch5); each field-manual copy "has a number cut into the board and the number is against a name in an issue book" (ch2); Keld's coat and token (ch3). Carry-forward 3 binds: only "a man from the Register" crosses. In ch10 the cell may speak of the Register only as a form. The lean toward Malphas is never connected to anyone.
- **C9: WARN ⇢ ch10 (H21).** "The hind has 'Eleven days of the ferment' on Day 1. Outline ch10 has her 'dead twenty-three days' on Day 12." "Ch10 should not print a number that fights ch2." Decided above: no age is printed.
- **Cast watch.** "Watch: Deming trips at ch10 unless ch10, where he is in the cast, gives him a fact." Give him one on the page.
- **C12: WARN.** Greyshaft Nine's history: "the seal broken on a quarterly inspection that had stopped coming nine years ago" against "The main under Greyshaft Nine had been capped four years". Print neither figure beside the other. Use "capped four years" if a figure is needed.
- **C14: NOTE.** "No chapter may say the rail's nine came from Ferriby."
- **C16: NOTE.** Units: divisions only.
- **C1: CRITICAL ⇢ ch44, carry-forward 8** (Wystan's side). Not on this page, but the lean must not be written as a law the cell states about *people*: Malphas's line is "the sink is the quiet", and nothing more.
- **C2, C3 (ch6), C4, C5 (ch7), C6 (ch9): CRITICAL, resolved in their chapters.** C3's reading stands: Malphas knew about the contraction; what Nuvalik concealed was the record in the book.
- **C7: CRITICAL ⇢ ch12** (Wystan's side; not this chapter).

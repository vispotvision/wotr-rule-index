...
| `publish_part.py` | **WAR-14, Phase 4.** Publishes `the-essence-ledger.md` as the Notion page *IX. The Essence Ledger (Part Twenty-Three)*, under **Fracture of Worlds — The Living System** beside volumes I to VIII. Reads the draft and never edits it: two named patches are applied in memory, each asserted to match exactly once, and the body that went up is kept at `_published/the-essence-ledger.notion.md`. Both patches exist to meet **clean publishing** (Isaac, 2026-09-26, `.claude/skills/wotr-write/references/fair-play.md`) — the draft's header becomes a standfirst in the page's own register, and §10's stale footnote about where the R44 rows sit is removed rather than corrected. The rest of the draft's apparatus (the markers, the file-and-line citations, `PENDING`, §9, §10) is WAR-12's text and is **not** rewritten here; that is a published edition of the Part and is its own issue. Markdown → blocks is `build/notion_publish.md_to_blocks`, the converter the rest of the wiki is published with. `_published_pages.json` holds the page id, so `--apply` replaces the body rather than making a second page. Writes nothing in `wiki/` — the hourly sync mirrors it. |

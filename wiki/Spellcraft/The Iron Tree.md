...
> ❓ Open question (SA-CROSS-IRONTREE-CRAFT-SHELF): Four of the twenty pages filed on the Spellcraft shelf declare a Craft that is not Spellcraft: The Iron Tree and Mugen no Hatsurugi are Magicraft, Precept is Runecraft, and Kami no Kobushi declines… This section may change when it is ruled.
> ❓ Open question (SA-GAP-IRONTREE-RECOMPRESSION): The Iron Tree's true rate limiter is not the EU cost but the recompression schedule, and the page states no schedule: no time, no rate, no condition. This section may change when it is ruled.
> ❓ Open question (SA-NUM-IRONTREE-XGRADE-VERSUS-OUTPUT): One page states both. This section may change when it is ruled.
> ❓ Open question (SA-UNATT-DOUGOU-CRYSTAL-STATE): 'Sealed near-total' is not one of the three Crystal States. This section may change when it is ruled.
...
seal.
1. `wiki/Volume I — Character Cards/Dougou Ozumu Zettari.md` — the **92,000 EU
